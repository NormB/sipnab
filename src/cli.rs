...
impl Cli {
    ...
    pub fn warn_unimplemented_flags(&self) {
        ...
        if self.dialog_track.is_some() {
            warnings.push((
                "--dialog-track",
                "not yet implemented; using default Call-ID tracking",
            ));
        }
        ...
        // TLS flags when tls feature is not compiled in
        #[cfg(not(feature = "tls"))]
        {
            ...
            if self.dtls_keylog.is_some() {
                warnings.push((
                    "--dtls-keylog",
                    "requires the 'tls' feature (not compiled in)",
                ));
            }
            if self.srtp_keys.is_some() {
                warnings.push((
                    "--srtp-keys",
                    "requires the 'tls' feature (not compiled in)",
                ));
        ...
        // API flags when api feature is not compiled in
        #[cfg(not(feature = "api"))]
        {
            if self.api.is_some() {
        ...
        // --metrics is not yet wired to an endpoint regardless of features
        if self.metrics.is_some() {
            warnings.push((
                "--metrics",
                "not yet implemented (Prometheus endpoint planned for a future release)",
            ));
        }

        for (flag, detail) in &warnings {
            eprintln!("warning: {flag} is {detail}");
        }
    }
}
