...
pub struct App {
    ...
    /// The session's archive passwords: the configured ones and the ones
    /// that opened an archive of the capture on screen. A load borrows it for
    /// its thread and gives it back. Cleared when another capture opens and
    /// when the session ends.
    #[cfg(feature = "archive")]
    archive_keyring: SessionKeyring,
}

/// A TUI session's archive keyring, shared with the load thread that borrows
/// it.
#[cfg(feature = "archive")]
pub(crate) type SessionKeyring =
    Arc<parking_lot::Mutex<Option<crate::capture::archive::password::Keyring>>>;
