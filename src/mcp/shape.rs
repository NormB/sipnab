...
// The compound-cursor rule (parse, format, and the identity tie-break) now
// lives in `crate::cursor`, outside the `mcp` feature so the REST
// `/v1/dialogs/tail` route resumes from the same implementation. Re-exported
// here so the tool call sites keep reading `super::shape::parse_cursor`.
pub use crate::cursor::{CURSOR_SEP, Cursor, format_cursor, parse_cursor};
