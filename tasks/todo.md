...
- [ ] src/sip/dialog.rs:348 — [edge-case] `update_invite_state` has the same `sub_state.starts_with("terminated")` false-positive fixed in timing.rs:117 (matches `terminatedfoo`); apply the exact-token match. *Found during the 2026-07-24 P2 wave.*
- [ ] src/tui/controllers/file_open.rs:411 — [doc-staleness] comment says the shared converter "clamps an out-of-range tv_usec"; after the file.rs:249 unification it rejects+counts+warns instead. Update the wording. *Found during the 2026-07-24 P2 wave.*
