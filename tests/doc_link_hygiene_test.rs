...
/// markdown page it does not skip as generated, and the rustdoc of `src/`.
///
/// The SAME scope as the fixer, on purpose. The gate used to read `docs/`
/// alone while the fixer covered every page, so an unlinked citation in
/// CHANGELOG.md committed clean -- and then the fixer's idempotence test,
...
fn rfc_link_scope() -> Vec<(String, String)> {
    ...
    for path in listed("*.md") {
        ...
        if head.contains("Generated by") || head.to_lowercase().contains("do not edit") {
            continue;
        }
        out.push((path, text));
    }
...
fn rfc_section_citations_are_linked() {
    ...
    for (path, text) in rfc_link_scope() {
        ...
        for (lineno, line) in prose_lines(&text) {
            ...
            for c in cite.captures_iter(&prose) {
                total += 1;
                offenders.push(format!(
                    "{path}:{lineno}: RFC {} section {} is not linked",
                    &c[1], &c[2]
                ));
            }
        }
    ...
    assert!(
        scanned > 400,
        "scanned only {scanned} files; the scope stopped matching the fixer's"
    );
