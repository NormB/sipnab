...
fn no_documentation_table_repeats_a_row() {
    ...
    // 207 -> 208 by tests/pcap-samples/PROVENANCE.md, the record of where each
    // committed capture fixture came from and what it pins (LIVE2). ONE file:
    ...
    // 217 -> 218 by the PAR1 surface-capability inventory
    // (surface-capability-inventory.md), generated by scripts/capability-matrix.py.
    // One new file in docs/design/, no website mirror.
    ...
    // 834 -> 836 by the `contact_rewrite` field table under
    // `describe_endpoint` in docs/mcp-tools.md and its site mirror: one
    ...
    // 891 -> 952 by PAR2: the surface-capability matrix
    ...
    // 934 -> 935: the "Operator notes on frames" capability in
    // docs/design/surface-capability-matrix.md, one `| Surface | Detail |`
    // table in a design doc with no site mirror. Measured by this gate.
    const EXPECTED_TABLES: usize = 935;
