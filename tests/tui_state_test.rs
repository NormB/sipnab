...
mod tui_state {
    ...
    /// Focus index of the filter dialog's "All" master checkbox, which follows
    /// the text fields.
    fn all_focus() -> usize {
        App::filter_text_field_count_for_test()
    }

    /// Focus index of method checkbox `k` (0 = REGISTER, 1 = OPTIONS, ...).
    fn checkbox_focus(k: usize) -> usize {
        all_focus() + 1 + k
    }

    /// Focus index of the first button, after the ten method checkboxes.
    fn buttons_focus() -> usize {
        checkbox_focus(10)
    }
    ...
    fn filter_all_checkbox_from_mixed_state_enables_all_first() {
        ...
        for _ in 0..checkbox_focus(INVITE_IDX) {
            app.handle_key(KeyCode::Tab);
        }
        app.handle_key(KeyCode::Char(' '));
        let (_, methods) = app.filter_focus_and_methods_for_test();
        ...
        // Back up to the All checkbox (above the grid) and toggle.
        for _ in 0..checkbox_focus(INVITE_IDX) - all_focus() {
    ...
    fn filter_right_column_reachable_by_tab_and_toggle() {
        ...
        assert_eq!(
            focus,
            checkbox_focus(1),
            "Tabs should land on right-column checkbox 1 (OPTIONS)"
        );
        app.handle_key(KeyCode::Char(' '));
        let (_, methods) = app.filter_focus_and_methods_for_test();
        assert!(
    ...
    fn filter_down_arrow_reaches_second_column() {
        ...
        // Down 4 times walks the left column to INFO (idx 8).
        for _ in 0..4 {
    ...
    fn filter_right_arrow_reaches_second_column() {
        ...
        assert_eq!(app.filter_focus_and_methods_for_test().0, checkbox_focus(0));
        // Right arrow should move into the second column (checkbox 1).
    ...
    fn filter_checkbox_down_moves_by_row() {
        ...
        // Tab through the text fields and the All row to the first method
        // (REGISTER).
        for _ in 0..checkbox_focus(0) {
            app.handle_key(KeyCode::Tab);
        }
        ...
        // Down should go to INVITE (idx 2), not OPTIONS (idx 1)
        app.handle_key(KeyCode::Down);
        assert_eq!(app.filter_dialog.focused_field(), checkbox_focus(2)); // INVITE

        // Down again -> SUBSCRIBE (idx 4)
        app.handle_key(KeyCode::Down);
        assert_eq!(app.filter_dialog.focused_field(), checkbox_focus(4)); // SUBSCRIBE

        // Down again -> NOTIFY (idx 6)
        app.handle_key(KeyCode::Down);
        assert_eq!(app.filter_dialog.focused_field(), checkbox_focus(6)); // NOTIFY

        // Down again -> INFO (idx 8)
        app.handle_key(KeyCode::Down);
        assert_eq!(app.filter_dialog.focused_field(), checkbox_focus(8)); // INFO

        // Down from the bottom of the LEFT column continues into the RIGHT
        // column (OPTIONS, idx 1) so it's reachable by vertical nav.
        app.handle_key(KeyCode::Down);
        assert_eq!(app.filter_dialog.focused_field(), checkbox_focus(1)); // OPTIONS
        // ...down the right column: PUBLISH(3) MESSAGE(5) REFER(7) UPDATE(9)
        for k in [3, 5, 7, 9] {
            app.handle_key(KeyCode::Down);
            assert_eq!(app.filter_dialog.focused_field(), checkbox_focus(k));
        }
    ...
    fn filter_checkbox_right_moves_by_column() {
        ...
        assert_eq!(app.filter_dialog.focused_field(), checkbox_focus(0)); // REGISTER (left col)

        // Right should go to OPTIONS (idx 1)
        app.handle_key(KeyCode::Right);
        assert_eq!(app.filter_dialog.focused_field(), checkbox_focus(1)); // OPTIONS

        // Right again from right column — no-op
        app.handle_key(KeyCode::Right);
        assert_eq!(app.filter_dialog.focused_field(), checkbox_focus(1)); // still OPTIONS
    ...
    fn filter_checkbox_up_moves_by_row() {
        ...
        // Up should go back to REGISTER (idx 0)
        app.handle_key(KeyCode::Up);
        assert_eq!(app.filter_dialog.focused_field(), checkbox_focus(0)); // REGISTER

        // Up from the top method row -> the All checkbox
        app.handle_key(KeyCode::Up);
        assert_eq!(app.filter_dialog.focused_field(), all_focus()); // All
