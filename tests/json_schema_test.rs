...
fn every_object_in_a_sipnab_schema_is_closed_at_every_depth() {
    ...
    // Six: message, dialog, call_report and stream; capture_analysis, the
    // capture-level analysis `--json-analyze`, `GET /v1/report` and the MCP
    // `get_capture_report` all serialize; and `notes.schema.json`, the
    // operator notes file.
    assert_eq!(
        checked, 6,
