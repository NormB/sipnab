//! CLI golden tests (verification plan M1 — T1.4).
//!
//! Declarative process snapshots via `trycmd`: each `tests/cli/cmd/*.trycmd`
//! case pins a command's combined stdout/stderr and exit code. Cases run under
//! the determinism contract (spec §4d): `TZ=UTC`, `NO_COLOR=1`, fixed terminal
//! size — so output is stable across machines, locales, and TTY state.
//!
