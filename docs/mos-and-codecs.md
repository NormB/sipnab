...
| **AMR narrowband** | **Nothing is published** | Placeholder, flagged |
...
large enough that the mode matters more than most of the impairments being
modelled:

### Monotic — handset or monaural headset (G.113 Table IV.1)
...
### Diotic — stereo headset or speakerphone (G.113 Table IV.3)
...
- **23.85 kbit/s scores worse than the slower 23.05.** `Ie,WB` is 8 against 1.
  The inversion recurs across Tables IV.1, IV.3 and IV.4, so it is published
  intent. Do not rank AMR-WB modes by bitrate and expect quality to follow.
...
So **AMR-WB with packet loss on a handset cannot be scored** from published
data. sipnab reports that rather than borrowing the diotic figure, which would
...
## AMR narrowband — nothing is published
...
`Ie,fb ≈ Σ Ie,wb + 19` is stated in one direction only; running it backwards to
manufacture a wideband EVS value is prohibited interpolation, and it would drag
the diotic assumption across with it.
...
> G.113 (2024) Erratum 1 (12 January 2026) to **32 kbit/s**. A pipeline built
