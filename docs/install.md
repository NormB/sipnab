...
```bash
curl -fsSL https://sipnab.com/install.sh | SIPNAB_VERSION=0.5.183 sh
```
...
|---|---|---|---|---|
| `*-linux-musl.tar.gz` | statically linked, 1.10.6 | compiled in; 0.5.183 and earlier speak only netmap API 13 (see below) | no | no |
...
captures through `netmap:` on that host. The musl build moves to those
headers, and the fix ships in the next release. Until you run it, treat the
musl netmap column as not usable.
...
Alpine included. Replace `<version>` with the latest, e.g. 0.5.183:
...
```bash
sudo rpm -i sipnab-0.5.183-1.x86_64.rpm
```

The headless / no-ALSA variant on the same architecture:

```bash
sudo rpm -i sipnab-0.5.183-1.x86_64-noaudio.rpm
```
...
```bash
sudo rpm -i sipnab-0.5.183-1.aarch64.rpm
```

The headless / no-ALSA variant on aarch64:

```bash
sudo rpm -i sipnab-0.5.183-1.aarch64-noaudio.rpm
```
...
```text
sipnab 0.5.183 (<hash>) features: native,tui,audio,tls,hep,api,mcp,mcp-http,metrics,plugins,bpf,vcon
libpcap version 1.10.5 (with TPACKET_V3); alternate capture backends named: none
```
