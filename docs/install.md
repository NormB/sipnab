...
with `SIPNAB_VERSION=0.5.27`, change the destination with `SIPNAB_INSTALL_DIR`.
...
# Replace <version> with the latest, e.g. 0.5.27
...
```bash
# amd64 (x86_64) -- replace <version> with the latest, e.g. 0.5.27
...
```bash
sudo rpm -i sipnab-0.5.27-1.x86_64.rpm
# headless / no-ALSA variant:
sudo rpm -i sipnab-0.5.27-1.x86_64-noaudio.rpm
# arm64 hosts:
sudo rpm -i sipnab-0.5.27-1.aarch64.rpm
```
...
```
sipnab 0.5.27 (<hash>) features: native,tui,audio,tls,hep,api,mcp,mcp-http
```
