...
```bash
curl -fsSL https://sipnab.com/install.sh | SIPNAB_VERSION=0.5.145 sh
```
...
Alpine included. Replace `<version>` with the latest, e.g. 0.5.145:
...
```bash
sudo rpm -i sipnab-0.5.145-1.x86_64.rpm
```

The headless / no-ALSA variant on the same architecture:

```bash
sudo rpm -i sipnab-0.5.145-1.x86_64-noaudio.rpm
```
...
```bash
sudo rpm -i sipnab-0.5.145-1.aarch64.rpm
```

The headless / no-ALSA variant on aarch64:

```bash
sudo rpm -i sipnab-0.5.145-1.aarch64-noaudio.rpm
```
...
Target binary size (musl, stripped): <= 14 MB. Enforced against the real artifact by the "Enforce published binary size" step in release.yml.
...
```text
sipnab 0.5.146 (<hash>) features: native,tui,audio,tls,hep,api,mcp,mcp-http,metrics,plugins,vcon,bpf
```
