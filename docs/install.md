...
with `SIPNAB_VERSION=0.5.3`, change the destination with `SIPNAB_INSTALL_DIR`.
...
# Replace <version> with the latest, e.g. 0.5.3
...
```bash
# amd64 (x86_64) -- replace <version> with the latest, e.g. 0.5.3
...
```
sipnab 0.5.3 (2595058) features: native,tui,audio,tls,hep,api,mcp,mcp-http
```
