...
```bash
curl -fsSL https://sipnab.com/install.sh | SIPNAB_VERSION=0.5.184 sh
```
...
Alpine included. Replace `<version>` with the latest, e.g. 0.5.184:
...
```bash
sudo rpm -i sipnab-0.5.184-1.x86_64.rpm
```

The headless / no-ALSA variant on the same architecture:

```bash
sudo rpm -i sipnab-0.5.184-1.x86_64-noaudio.rpm
```
...
```bash
sudo rpm -i sipnab-0.5.184-1.aarch64.rpm
```

The headless / no-ALSA variant on aarch64:

```bash
sudo rpm -i sipnab-0.5.184-1.aarch64-noaudio.rpm
```
