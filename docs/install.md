...
```bash
curl -fsSL https://www.sipnab.com/install.sh | SIPNAB_VERSION=0.5.89 sh
```
...
Alpine included. Replace `<version>` with the latest, e.g. 0.5.89:
...
```bash
sudo rpm -i sipnab-0.5.89-1.x86_64.rpm
```

The headless / no-ALSA variant on the same architecture:

```bash
sudo rpm -i sipnab-0.5.89-1.x86_64-noaudio.rpm
```
...
```bash
sudo rpm -i sipnab-0.5.89-1.aarch64.rpm
```

The headless / no-ALSA variant on aarch64:

```bash
sudo rpm -i sipnab-0.5.89-1.aarch64-noaudio.rpm
```
...
```text
sipnab 0.5.90 (<hash>) features: native,tui,audio,tls,hep,api,mcp,mcp-http,metrics
```
