...
sipnab dispatches on the method that opened the dialog. Three methods get their
own state machine and the rest share a generic one.

| Machine | Methods | Terminal states |
|---|---|---|
| INVITE | `INVITE` | `InCall`, `Completed`, `Cancelled`, `Failed` |
| REGISTER | `REGISTER` | `Registered`, `Failed` |
| SUBSCRIBE | `SUBSCRIBE` | `Active`, `Terminated` |
| generic | the other eleven | `Completed`, `Failed` |
...
|---|---|---|---|
| `ACK` | generic | [RFC 3261 §17.1.1.3](https://www.rfc-editor.org/rfc/rfc3261#section-17.1.1.3) | This section specifies the construction of ACK requests sent within the client transaction. A UAC core that generates an ACK for 2xx MUST instead follow the rules described in Section 13. |
| `BYE` | generic | [RFC 3261 §15.1](https://www.rfc-editor.org/rfc/rfc3261#section-15.1) | A BYE request is constructed as would any other request within a dialog, as described in Section 12. Once the BYE is constructed, the UAC core creates a new non-INVITE client transaction, and passes it the BYE request. |
| `CANCEL` | generic | [RFC 3261 §9](https://www.rfc-editor.org/rfc/rfc3261#section-9) | The previous section has discussed general UA behavior for generating requests and processing responses for requests of all methods. In this section, we discuss a general purpose method, called CANCEL. |
...
| `PRACK` | generic | [RFC 3262 §6](https://www.rfc-editor.org/rfc/rfc3262#section-6) | This specification defines a new SIP method, PRACK. The semantics of this method are described above. |
