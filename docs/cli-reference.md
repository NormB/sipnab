...
- `sudo sipnab -N -I done.pcap --mcp --mcp-allow-tls-capture` — let an agent decide, mid-investigation, that the answer is in traffic it cannot see, and start reading TLS plaintext itself. The file source must be finished first: sipnab's stores have one writer, and a uprobe capture is a live one
