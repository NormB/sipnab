...
A first run needs only a handful of these flags:

- [`-I`, `--input`](#capture) reads packets from a capture file, a directory of them, an archive of them, or a glob, instead of live capture.
- [`-d`, `--device`](#capture) names the network interface to capture on.
- [`-N`, `--no-tui`](#mode) turns on non-interactive mode, with no TUI. The batch and output flags require it.
- [`--report`](#output) generates a summary report after capture completes. Requires `-N`.
- [`--json`](#output) prints output as NDJSON, one JSON object per line. Requires `-N`.
- [`--filter`](#matching) takes a Filter DSL expression or a diagnostic alias name, such as `codec-asym` or `late-media`.
...
The `--slow-setup` alias asks the same question against the run's own post-dial
delay threshold — 11 seconds, unless `--pdd-threshold` or
`[diagnosis] post_dial_delay_secs` moves it — so a quick check needs no filter
expression at all.
...
Audio export is a TUI workflow — see [Keybindings](keybindings.md#rtp-streams).
...
>
> sipnab deletes only the files the
...
> however closely its name resembles a rotation.
>
> A run that dies mid-capture
...
| `--portrange` | `<RANGE>` | `5060-5061` | SIP **signaling** port range. Media is never gated — RTP uses SDP-negotiated dynamic ports. The default is narrow and carriers routinely run SIP on 5070, 5080 and elsewhere, so widen it or analyze a fraction of the file — see the note below. HEP input (`-L`, `--hep-parse`) is never gated: the sender already chose what to send, and the ports in its HEP chunks are its claim, not something sipnab captured |
| `--ws-portrange` | `<RANGE>` | `80, 443, 8080, 8443` | Ports carrying SIP-over-WebSocket ([RFC 7118](https://www.rfc-editor.org/rfc/rfc7118)), as one inclusive `START-END` range in the same grammar as `--portrange`. The shipped set is the browser's view of the web, not a deployment's: Kamailio, OpenSIPS and Janus each default to WSS outside it, and behind a reverse proxy sipnab sees whichever port the proxy forwards to — so the whole WebRTC signaling leg stays invisible. A range **replaces** the shipped set, exactly as `--portrange` replaces the default signaling ports. sipnab counts the SIP-over-WebSocket it declines to unwrap and names the ports it arrived on. The range governs plain WebSocket only: sipnab reads a WebSocket frame it decrypts out of TLS with `--keylog` on any port and labels it WSS. It joins a frame split across TLS records (OpenSIPS writes the header and the payload as two), several frames in one record, and a message fragmented across frames, up to `MAX_WS_MESSAGE_SIZE` (65,536) bytes per message. It reads the HTTP upgrade and its `101` answer across TLS records too (OpenSIPS writes its `101` as three), up to `MAX_WS_HANDSHAKE_SIZE` (8,192) bytes, and frames start at the byte after the blank line that ends them, in the same record or a later one. A frame, message or upgrade it cannot finish or decode counts in the NOT DECODED line. Config: `[capture] ws_ports` |
...
>
> When the share is high, sipnab additionally
...
- **sipnab accounts for every member.** A member that is not a capture gets a
  ...
  resolved to` line counts them.

  A member whose link type sipnab does not
...
- **Decryption works on members exactly as on plain files.** `--keylog`, the
  secrets a pcapng embeds, SDES keys in the SDP and `--dtls-keylog` each decrypt
  a member the same whether it arrived plain, gzip-compressed, in a tar, in a
  `.tgz`, or gzip-compressed inside a `.tgz`.
...
| `--no-password-prompt` | -- | off | Never ask for an archive password on the terminal |
...
7. The prompt. When no configured password opens a member, sipnab asks on
   the terminal, the way ssh and sudo do: `Password for evidence.zip (member
   voip/call3.pcap, attempt 1 of 3):`. It reads `/dev/tty`, not stdin, so it
   still asks with stdin redirected, and it echoes nothing. Each archive gets
   three attempts. An empty entry, or Ctrl-D, skips that archive's locked
   ...
   [Archive password popup](keybindings.md#archive-password-popup).
...
**Exports carry the data out unencrypted.** `-O`, `--strip-secrets` and
`--write-annotated` from a member decrypted out of a password archive each warn
once that they write that data unencrypted. The section comment of a
`--write-annotated` copy says so too, and names no password.
...
- `sipnab -N -I evidence.zip --report` — no password configured: sipnab asks on the terminal for each locked archive, three tries each
- `sipnab -N -I evidence.zip --no-password-prompt --json-dialogs --no-cli-print` — in a script that must never wait on a keyboard: sipnab skips and counts the locked members instead
- `sipnab -N -I 'drop/*.zip' --archive-password-file pw.txt --no-password-prompt --report` — try the configured passwords and nothing else, even from a terminal
...
Two consequences follow, and both bite the obvious command lines. Adding
...
A note is a remark a person writes about one SIP message, carried to the
person who opens the capture next. sipnab writes each note into a pcapng
packet comment, where Wireshark shows it beside the frame. sipnab never reads a
packet comment back: a note is output, never part of the analysis.
...
Shortcut flags that expand to predefined filter DSL expressions. See [filter-dsl.md](filter-dsl.md#named-aliases) for the exact expansion of each alias.

| Flag | Value | Default | Description |
|------|-------|---------|-------------|
| `--problems` | -- | off | Show calls matching any diagnostic signal: failed state, one-way audio, RTP loss past `--loss-bad-pct` (5% by default), jitter past `--jitter-bad-ms` (50 ms by default), NAT mismatch, more than 3 retransmits, post-dial delay past `--pdd-threshold` (11 s by default), codec/ptime/payload/duration asymmetry, or late media. Every one of those thresholds tracks the run's own setting, so a network that tuned `[diagnosis]` or `[quality]` to its own SLA gets an alias that agrees with the rest of the run — see [Named Aliases](filter-dsl.md#named-aliases) for the exact expansion. Orphaned RTP is **not** among them: an orphaned stream belongs to no dialog, so it cannot select one. Find it in the "Orphaned Streams" section of `--report`, or `/v1/streams?orphaned=true` |
| `--slow-setup` | -- | off | Show calls whose post-dial delay passes `--pdd-threshold` (11 seconds by default, from `[diagnosis] post_dial_delay_secs`) |
| `--short-calls` | -- | off | Show completed calls shorter than `--fraud-short-call` (3 seconds by default, from `[security] fraud_short_call_secs`) |
...
| `--json-analyze` | -- | off | The `--analyze` result as one JSON object, emitted after capture. One object rather than a line per finding: the frames read and the dialogs examined are properties of the run, and a clean capture must still serialize to something that states them. Requires `-N` |
| `--yang-analyze` | -- | off | The `--analyze` result [RFC 7951](https://www.rfc-editor.org/rfc/rfc7951)-encoded: the same analysis `--json-analyze` prints, as one JSON document that validates against the `sipnab-diagnosis` YANG module. 64-bit integers are strings, names use hyphens, and each finding carries its `rank`. Asked for with `--json-analyze`, both come from one analysis. Requires `-N` |
| `--print-yang-module` | -- | off | Print the `sipnab-diagnosis` YANG 1.1 module and exit: the model the [RFC 7951](https://www.rfc-editor.org/rfc/rfc7951) export of the capture analysis validates against, byte for byte the file committed under `yang/`. Needs no capture, config or privileges |
...
| `--proto-number` | -- | off | Annotate the transport tag with the IANA IP protocol number, e.g. `UDP(17)` / `TCP(6)`. Long-only because `-N` is `--no-tui` here; TLS, WS and WSS report their TCP carrier's number (6) |
...
| `--lint` | -- | off | Run the RFC conformance linter over every dialog and print each finding with its rule identifier and the RFC section it reads from. The linter compares what the capture holds against what the cited section calls for, so a finding names a section rather than an opinion. Informational on its own: it changes what gets printed and never the exit code. Pair it with `--lint-fail-on` to make a pipeline stop. The rule catalog is in [sip-lint-rules.md](sip-lint-rules.md), and over MCP as `explain_rule` |
...
- `sipnab -N -I capture.pcap --export-vcon '1-1966@10.0.2.20' > call.vcon` -- hand one observed dialog to a conversation-data pipeline in the interchange format it already reads, instead of a sipnab-shaped JSON nobody else parses. The container names sipnab as an observer party and carries what this capture missed, so a consumer weighs it as an observation rather than as a recording
- `sipnab -N -I capture.pcap --export-vcon '1-1966@10.0.2.20' --vcon-out ./call.vcon --no-cli-print` -- the same container to a file, with the run otherwise silent. Use this spelling in a script: the exit code is the whole answer, and a path sipnab cannot write fails the run rather than leaving an operator to discover the missing file later
...
| `--active-idle-window` | `<SECS>` | `3600` | Seconds a dialog may go untouched and still count toward the active-dialog and active-call gauges every surface publishes. The shipped hour is twice [RFC 4028](https://www.rfc-editor.org/rfc/rfc4028)'s default `Session-Expires`, which grounds it for a trunk carrying session timers and not for a contact center, where a caller parked on hold past an hour is a channel in use the gauge stops counting. Widening it widens the opposite error -- a call that never sent its BYE keeps counting for longer, and that one never recovers on its own. Config: `[sip] active_idle_window_secs` |
...
> new branch ([RFC 3261 section 17.1.1.3](https://www.rfc-editor.org/rfc/rfc3261#section-17.1.1.3)) and the BYE another, so one ordinary call appears as
...
| `--quality-interval` | `<SECONDS>` | `5` | Seconds between RTP quality snapshots — the resolution of the per-stream quality trend, which carries a MOS, an R-factor and an `acceptable`/`degraded`/`not_scorable` verdict per interval. The shipped five seconds averages away any burst shorter than itself, so three half-second dropouts read as a mild five-second average. The trend still covers an hour of call time whatever this is, so one second retains 3600 snapshots per stream instead of 720: resolution costs memory, never history. Accepts 1 to 300. Config: `[limits] quality_interval_secs` |
...
- `sudo sipnab -N -d eth0 --rtpengine-control 127.0.0.1:22222 --relay-compare 1-7@203.0.113.9` — after capture, put the relay's own RTP packet count for the call beside sipnab's measured count, each labeled with its tier, with a `match`/`differ` verdict and a note explaining that an ordinary gap is not a relay fault. It shows both counts and never sums them
- `sipnab -N -I capture.pcap --relay-compare 1-7@203.0.113.9 --rtpengine-control 127.0.0.1:22222` — refused on a file run: comparing asks the relay for its side, and asking transmits, so it prints `not_permitted` rather than reaching an address the capture named
...
it would see green until the value was already bad.

A boundary that is not a
...
| `--reg-flood` | -- | off | Detect registration floods: credentialed REGISTERs the registrar keeps refusing. sipnab reports a source when the REGISTERs it sent with `Authorization` that drew a `401`/`407` on the same transaction exceed `--reg-flood-threshold` inside one `--reg-flood-window` of capture time, one second by default. A REGISTER on its own is never evidence, so a re-REGISTER storm the registrar accepts after a restart is the customer's SBC and not a flood, and any `2xx` to a REGISTER clears the source's count |
| `--reg-flood-threshold` | `<N>` | `50` | Challenged failures from one source inside one `--reg-flood-window` before `--reg-flood` reports a flood: REGISTERs that carried credentials and drew a `401` or `407`. The default is a carrier-registrar figure: it never sees the ten-a-second brute force a small PBX gets. Counted in capture time, so a file replays as the traffic it recorded rather than as fast as the disk reads it. Config: `[security] reg_flood_threshold` |
| `--reg-flood-window` | `<SECS>` | `1` | How much capture time one `--reg-flood` counting window spans, 1-3600 seconds. The threshold counts failures per window, so this decides how concentrated a credential-guessing run has to be: one refusal every two seconds never puts two inside the default one-second window, whatever the threshold. sipnab refuses `0` and anything past an hour. Config: `[security] reg_flood_window_secs` |
| `--reg-flood-transaction-timeout` | `<MS>` | `32000` | How long a credentialed REGISTER stays open to the `401`/`407` that answers it, 1000-600000 milliseconds. The default is [RFC 3261 Timer F](https://www.rfc-editor.org/rfc/rfc3261#section-17.1.2.2), 64 times T1 at the default T1 of 500 ms. sipnab never counts a challenge that arrives later as a failure. Set it to 64 times the T1 your network runs, or to your registrar-side proxy's final-response timer when that is longer: `fr_timeout` in OpenSIPS (seconds, so multiply by 1000) or `fr_timer` in Kamailio (already milliseconds). See [Registration-flood timers](troubleshooting.md#registration-flood-timers). Config: `[security] reg_flood_transaction_timeout_ms` |
...
> `s`/`m`/`h` suffix).
>
> Rule names are `scanner`, `fraud`, `digest` and
> `reg-flood` (also `reg_flood`). An unknown rule name exits 2 at startup.
> A rule counts detector findings, not raw packets.
>
> An unrecognized bare word draws a warning naming the
...
- `sudo sipnab -N -d eth0 --reg-flood --reg-flood-threshold 20 --reg-flood-window 60` — a slow credential-guessing run: twenty refused credentialed REGISTERs from one source inside a minute is a flood, even when no single second holds more than one
- `sipnab -N -I registrar.pcap --reg-flood --reg-flood-transaction-timeout 64000` — a network whose phones run T1 at one second, so RFC 3261 Timer F is 64 seconds and a registrar challenge forty seconds after the REGISTER still answers an open transaction
...
since a hook already runs as you.

It does mean a hook command is as trusted as
...
A hook that needs to act privileged should ask something that
...
| `-L`, `--hep-listen` | `<ADDR>` | -- | Listen for HEP (Homer Encapsulation Protocol) packets. **Combines with `-d <iface>`**: HEP then supplies signaling while the interface supplies the RTP a HEP feed cannot carry, and streams bind to dialogs by SDP media endpoint. With no BPF expression of your own, the interface gets a media-only default, since the mirror carries the signaling (up to 0.5.192 it got the signaling filter, captured no media and saw every mirrored message a second time); an explicit expression such as `sipnab -N -d eth0 -L 127.0.0.1:9060 "udp portrange 10000-20000"` narrows it to your media ports. One interface and one listener only: `--multi-device` with `-L`, `-I` with `-L`, and `-O` alongside the pair are each refused with the reason. See [cookbook recipe 6d](examples.md#6d-take-signaling-from-hep-and-media-off-the-wire-in-one-process). The decrypted TLS and WebSocket legs OpenSIPS and Kamailio trace arrive with IP protocol 22, 50 or 6 and decode as TLS, WS or WSS, see [How `--hep-listen` reads the transport](#how---hep-listen-reads-the-transport). `--portrange` does not apply to HEP messages, so a proxy tracing SIP on 7060 needs no `--portrange 1-65535`. Feature: `hep` |
...
| `--hep-silence-warn` | `<SECS>` | `30` | Seconds a `--hep-listen` listener may go without **admitting** a packet before it logs a warning; `0` turns the warning off. Only an admitted packet counts as traffic. When packets keep arriving and sipnab refuses every one, the warning says `no packets admitted` and names the refusal most of them met (`auth_mismatch`, `allowlist`, `hmac_timestamp_out_of_window` and so on) and a peer that sent them: the sender is reaching the port and sipnab is turning it away, which is a key, mode or allowlist problem rather than a network one. When nothing arrives at all the warning keeps its `no packets for` wording. Refused without `--hep-listen`, which is the side it governs. Feature: `hep` |
| `--hep-senders` | -- | off | At the end of a headless (`-N`) run, print who fed the `--hep-listen` listener: each sender, keyed by the capture id it claims and the address it sent from, with its packet count, when sipnab last heard from it, how long it has been idle and whether it went silent past `--hep-silence-warn`; then every address the listener refused, with counts by reason, and the refusal totals. The capture id is the sender's claim, never proven. With `--json`, the same as one JSON object on the last line of stdout, the shape `GET /v1/hep/senders` and the MCP `hep_senders` tool return. The roster covers this run only and nothing persists it. Refused without `--hep-listen`, which is the side it describes. Feature: `hep` |
| `-E`, `--hep-parse` | -- | off | Parse incoming HEP packets (enable HEP decoding): sipnab unwraps a HEP datagram on the wire or in a capture file and reads its message with the addresses the HEP chunks carry. The transport comes from the IP protocol chunk by the same rule `--hep-listen` uses, so a TLS, WS or WSS leg an OpenSIPS or Kamailio tracer sends reads the same from a file as from a socket, see [How `--hep-listen` reads the transport](#how---hep-listen-reads-the-transport). `--portrange` does not apply to the HEP messages it unwraps. It still gates SIP that sipnab captures directly. Feature: `hep` |
...
| `--token-scope` | `<full\|metrics\|read>` | `full` | Scope for `--mint-token`. `metrics` reaches `GET /metrics` and returns `401` everywhere else — mint one for a scrape job rather than a credential that also reads `/v1/dialogs` and the message bodies underneath. `read` is the MCP counterpart: it reaches the tools annotated read-only and refuses every tool that writes or replaces state (`export_capture`, `open_capture`, `shutdown_server`, `tfps_ban` and the rest; `tools/list` marks each one with `readOnlyHint: false`). A cross-surface mint fails at mint time, so `metrics` with MCP and `read` with the REST API are both refused rather than issued and then rejected. |
...
- `sipnab -N -I today.pcap --api 127.0.0.1:8080 --api-key s3cret --api-file-root /srv/pcaps --archive-password-file /etc/sipnab/pcaps.pw` — the compare route opens password-protected ZIPs under `/srv/pcaps` with the operator's passwords, and a local client may add one per request in the `Sipnab-Archive-Password` header
- `sipnab -N -I today.pcap --api 0.0.0.0:8080 --api-key s3cret --api-file-root /srv/pcaps --api-accept-archive-passwords` — behind a TLS proxy on another host: remote clients may send `Sipnab-Archive-Password`, which without the flag get `403` because the password would cross the network in the clear
- `sipnab -N -I today.pcap --api 127.0.0.1:8080 --api-key s3cret --api-file-root /srv/pcaps --api-accept-archive-passwords --archive-password-file /etc/sipnab/pcaps.pw` — both at once: configured passwords first, then the request's
...
- `sipnab -N -L 0.0.0.0:9060 --hep-parse --hep-auth-file /etc/sipnab/hep.key --hep-silence-warn 300` — a collector fed by PBXes that go quiet overnight: five minutes without an admitted packet before the listener warns, so a site with no calls overnight does not fill the journal
- `sipnab -N -L 127.0.0.1:9060 --hep-parse --hep-silence-warn 0` — a lab listener fed by hand, where long silences are normal and the warning would only be noise
- `sipnab -N -L 0.0.0.0:9060 --hep-parse --hep-auth-file /etc/sipnab/hep.key --duration 300 --hep-senders` — five minutes of a collector's feed, then the roster: which proxies sent and how much, which went silent, and which addresses the listener turned away and why
- `sipnab -N -L 127.0.0.1:9060 --hep-parse --hep-senders --count 5000 --json` — the same roster as one JSON object on the last line of stdout, after the per-message lines, for a script to pick up with `tail -1`
...
Each datagram's IP protocol chunk carries the transport the message arrived
on: 17 for UDP, 6 for TCP, TLS and WebSocket, 132 for SCTP. A collector
filtering by transport finds a TCP trunk under TCP.
...
**RTP is never forwarded.** RTCP is a control channel that [RFC 3550 section 6.2](https://www.rfc-editor.org/rfc/rfc3550#section-6.2)
holds to a small fraction of session bandwidth, so it carries the quality
...
A HEP packet names the transport of the message it carries in its IP protocol
chunk (`0x0002`). The [HEP3 specification](https://github.com/sipcapture/hep/blob/83451db00e99f43b24dc68a391c0a7b2bb749b02/docs/HEP3_Network_Protocol_Specification_REV_37.pdf)
defines that chunk as the IP protocol ID, and sipnab reads 17 as UDP, 6 as TCP
and 132 as SCTP.
...
DECODED line by that number, as a listener does.
...
| `--mcp-resource-url` | `<URL>` | -- | Public URL clients reach this MCP server at, published as the OAuth 2.0 protected-resource identifier ([RFC 9728](https://www.rfc-editor.org/rfc/rfc9728.html)). Setting it turns on discovery: the `401` challenge gains a `resource_metadata` parameter and sipnab serves the metadata document unauthenticated at the path [RFC 9728 section 3.1](https://www.rfc-editor.org/rfc/rfc9728#section-3.1) derives from this value, so `https://sipnab.example.com/mcp` publishes at `/.well-known/oauth-protected-resource/mcp`. Leave it unset and the challenge still appears — only the metadata half is off. The operator names it rather than sipnab guessing: behind the TLS-terminating proxy this deployment expects, sipnab sees a cleartext request and no scheme, and anything derived from the socket or the `Host` header would name `http://` for a resource the client reached over `https://` — which [RFC 9728 section 3.3](https://www.rfc-editor.org/rfc/rfc9728#section-3.3) makes a conformant client discard. Discovery only: sipnab issues and validates no OAuth tokens, so the document advertises no `authorization_servers`, and the bearer tokens it accepts are still its own. Must be absolute, `http` or `https`, with no userinfo, query or fragment; a rejected value is a startup error. Feature: `mcp-http` |
...
| `--keylog` | `<FILE>` | -- | TLS key log file (NSS `SSLKEYLOGFILE` format). Accepts a **FIFO** here and reads it as a live stream, so a producer can feed secrets in without writing them to disk. sipnab opens the file **before** it drops privileges, so a keylog only the proxy's user can read still loads when sipnab runs as root and drops to `nobody`, and `--keylog-watch` keeps reading what the producer appends through that open file. A producer that replaces the file after the drop needs it reachable by the user sipnab dropped to. When sipnab cannot read the keylog, its error names the reason, such as `No such file or directory`. Feature: `tls` |
...
| `--uprobe-flavor` | `<NAME>` | all | Probe only these flavors (`openssl`, `wolfssl`); repeatable. Feature: `native`. Also accepts `--uprobe-flavour`, the spelling the flag shipped with through 0.5.104: a released flag is a contract rather than a spelling choice |
...
- `sipnab -N -I capture.pcap --mcp --mcp-tools core` — a small-context agent: nine tools instead of the whole surface, so the schema block sipnab sends it before it asks anything is a fraction of the size
