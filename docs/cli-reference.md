...
- `sipnab -N -I trunk.pcap --fraud-detect --fraud-short-call 6 --fraud-wangiri-calls 5 --fraud-sequential-calls 6`
- `sipnab -d eth0 -N --kill-scanner --evidence-out - | tfps_ctl ingest` — the split the two projects exist for: sipnab publishes what it saw, and the firewall applies its own ignore list, its own duration and its own audit to it
...
- `sipnab -N -I trunk.pcap --fraud-detect --fraud-destination CU,KP --json` — replay a trunk capture and write every INVITE to an embargoed destination as one JSON line, for the audit that asked whether any call ever went there — audit a wholesale trunk where short calls are ordinary, so a lure needs five of them and a dial-plan walk needs six consecutive dead numbers
...
- `sipnab --mcp -i eth0 --mcp-max-wait-seconds 300` — let an agent sit on a live capture for up to five minutes per `await_condition` call, on a box watching for a fault that reproduces slowly
- `sipnab --mcp -i eth0 --mcp-max-wait-seconds 5` — hold every wait to five seconds, for a host where MCP slots are scarce and a stalled agent must not occupy one
