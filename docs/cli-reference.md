...
| `--rtpengine-control` | `<ADDR>` | -- | Ask an rtpengine relay which calls it currently has up, so a capture that started mid-call can still name the dialog behind a stream. **Off unless you give an address**, and the address is never inferred from captured traffic. Only rtpengine's read-only `list` and `query` are reachable from this path — there is no value in the code that means `delete` or `start recording`, so sipnab cannot change a relay's behavior through it. **Not a poller**: it asks once, at startup, before the capture opens — never on a timer, and never again during the run. **Refused on `-I <file>`**: a live relay would answer about calls that are up TODAY, which are not the calls in the capture. Feature: `native` |
...
- `sudo sipnab -N -d eth0 --rtpengine-control 127.0.0.1:2223` — start capturing on a proxy that shares a host with rtpengine, and ask the relay once, at startup, which calls are already up. Streams whose offer happened before sipnab started get a Call-ID, where otherwise sipnab would report them as orphans
- `sudo sipnab -d eth0 --rtpengine-control 192.0.2.50:2223 --max-streams 10000` — the relay on its own host: same question, over the network to its ng control port. sipnab sends only `list` and `query`, once each per call the relay names, and then stops asking for the rest of the run
- `sipnab -N -I capture.pcap --rtpengine-control 127.0.0.1:2223` — refused, and says so: reading a file, sipnab never transmits, and the relay's answer would describe today's calls rather than the capture's
