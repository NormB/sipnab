...
| `--keylog` | `<FILE>` | -- | TLS key log file (NSS `SSLKEYLOGFILE` format). A **FIFO** is accepted here and read as a live stream, so a producer can feed secrets in with nothing written to disk. Feature: `tls` |
| `--keylog-fd` | `<N>` | -- | Read NSS keylog lines from an already-open descriptor, for a privileged producer that hands secrets over a pipe rather than a file. **Implies `--keylog-watch`**, and cannot be combined with `--keylog`. sipnab **cannot start that producer itself**: `PR_SET_NO_NEW_PRIVS` is set at startup and inherited by every child, so a child can never acquire the `CAP_BPF` an eBPF extractor needs — start it from a supervisor and pass the read end here. Feature: `tls` |
