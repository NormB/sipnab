...
| `--uprobe-symbol` | `<NAME>` | per flavour | Write symbol to probe. Defaults to the one the library's flavour exports — `SSL_write` for OpenSSL, `wolfSSL_write` for wolfSSL — so it is needed only for a library sipnab cannot classify by name. Feature: `native` |
...
- `sudo sipnab -N --uprobe-tls` — read SIP over TLS with **no certificate, no key and no restart of the SIP daemon**. Every mapped TLS library is probed, so a host running OpenSSL for one daemon and wolfSSL for another is covered by one command
...
- `sudo sipnab -N --uprobe-tls --uprobe-library /usr/lib/libssl.so.3 --uprobe-symbol SSL_write_ex` — probe OpenSSL 3's newer write entry point instead of the default `SSL_write`, for a daemon built against it; the argument positions are the same, so the probe shape is unchanged
