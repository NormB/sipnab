...
- Both counters zero does not mean the capture is complete: a frame can arrive intact and still be unreadable, or be cut short by `--snaplen`. `--report` counts those separately. See [tuning](tuning-capture.md).
