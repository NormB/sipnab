...
| [`doc_commands_run_test`](../../tests/doc_commands_run_test.rs) | A command shown in `docs/` is one sipnab refuses. It extracts every `sipnab …` line from every shell block, RUNS the ones it can against a capture that ships with the repository, and fails on a usage error rather than on a non-zero exit -- plenty of documented commands exit non-zero for honest reasons. It skips the rest under one of four named reasons (a capture device, root, a server that does not exit, a shell program), and a command matching none of them fails the test rather than slipping through. It prints its own coverage, because the difference between "232 ran" and "3 ran, 229 skipped" is the difference between a gate and a decoration. |
