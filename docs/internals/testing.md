...
| [`tests/pcap-samples/`](../../tests/pcap-samples) — capture fixtures | Ten are synthetic: `python3 tests/gen-pcap-samples.py` rewrites them, and the same script in check mode reports any that have drifted from it. Nothing regenerates the rest; they stay as checked in. |
