...
| **CLI golden** | [`cli_goldens`](../../tests/cli_goldens.rs) | `trycmd` goldens under [`tests/cli/`](../../tests/cli) — exact stdout/stderr for a command line. [`tests/cli/cookbook/`](../../tests/cli/cookbook) pins the output of every cookbook command [`check-cookbook.py`](../../scripts/check-cookbook.py) executes, one case per distinct command, run in a scratch directory because recipes write files. |
...
| [`teardown.rs`](../../tests/support/teardown.rs) | `terminate()`, the one way a harness stops the binary it spawned: SIGTERM, a bounded wait, and SIGKILL only for a child still running after it. `Child::kill()` is SIGKILL, and a process killed that way never writes its coverage profile. Pulled in with `include!` rather than `#[path]`. |
...
|---|---|
| [`tests/fixtures/`](../../tests/fixtures) — capture fixtures | Every capture in the directory comes from [`tests/support/synthetic_captures.rs`](../../tests/support/synthetic_captures.rs), and so do the two SIPp media files in `harness/sipp/scenarios/` and the two fuzz seeds in `fuzz/corpus/pcap_reader/` named `truncated-sip` and `empty-classic`: seventeen in all. `cargo run --features native --bin gen_fixture` writes them, and `synthetic_captures_test` fails when a committed byte differs from what the generator builds. The directory's other fixtures, the JSON goldens, `ng` replies and TLS keys, stay as checked in. [`tests/PROVENANCE.md`](../../tests/PROVENANCE.md) records where every capture outside `tests/pcap-samples/` came from, and `every_committed_capture_is_public_or_synthetic` refuses one with no entry, finding captures by their leading bytes rather than their names. |
...
| [`tests/cli/cookbook/`](../../tests/cli/cookbook) — cookbook output goldens | `python3 scripts/check-cookbook.py --bless` writes a command-only case for each executed cookbook command that has none and deletes each case no command produces; then `TRYCMD=overwrite cargo test --features full --test cli_goldens` records the output. A case's file name is a hash of its command, and it is also the directory the command's output files go to. |
...
| [`cookbook_recipes_test`](../../tests/cookbook_recipes_test.rs) | A command in [`docs/examples.md`](../../docs/examples.md) no longer runs or names a flag `--help` lacks; or a command it executes has no output golden in [`tests/cli/cookbook/`](../../tests/cli/cookbook) and no reason in the checker's `OUTPUT_UNPINNED` table; or a golden there is one no cookbook command produces. |
