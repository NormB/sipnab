...
and it did not fit. The ceiling `release.yml` enforces is 15 MB now, so it
...
- **`features`** — `cargo check --no-default-features --features X --tests`
  ...
  `native,tui,tls,hep,api` runs `tui_snapshot_test` (no `audio`). `bpf` is here because every published
...
them reports "nothing past the tag" and passes without looking. The hook also
...
bug rather than a limitation. Rustdoc lints and the
...
[`scripts/publish-crates.py`](../../scripts/publish-crates.py). The script
