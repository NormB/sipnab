...
(`binary_size_ceiling_mb` in [`website/config.toml`](../../website/config.toml) holds the current figure),
...
On macOS `Cargo.toml` declares `aya`
under `[target.'cfg(target_os = "linux")'.dependencies]`, so the feature would
compile to nothing while `--version` claimed it.
...
| `vcon` | `native` | vCon export: one observed dialog as an unsigned OBSERVER conversation container. **Non-default on purpose** — the stock build gains no conversation-container writer at all. It sits INSIDE `full`, so the pre-commit hook's `cargo test --features full` covers it. sipnab writes the container as an observer: nothing signed, no party name, and no `url` — retained audio travels INLINE as base64url or not at all, under a measured size budget the emitter refuses out loud. |
...
handed from one thread to another reads as a data race.
...
passes), but two things make that grep mean something.
...
run's "0 races" meant "nothing printed".

And the verdict names the finding
...
That had it backwards: `bootstrap::launch` spawns the
...
Those paths go through `capture::stop_and_join`, which sets
...
The inline version had to move because it was
...
job failed silently when clean and passed while a thread leak was present.
...
last manual sync, so a green local run says nothing about CI.
...
[`scripts/coverage.sh`](../../scripts/coverage.sh) reproduces that job locally, before a push:
...
`ci-success` requires every other job in `ci.yml` — thirteen of them: **`check`,
`install-sh`, `deb-package`, `rpm-packaging`, `homebrew-formula`,
`tsan-verdict`, `bench-scripts`, `features`, `crate-package`, `audit`,
`fuzz-check`, `code-scanning-clean`, `symbol-split`**.

`ci_success_gates_every_job` in
[`site_journey_test`](../../tests/site_journey_test.rs) holds the list to that,
...
one required check on main still green.

The four the rest of this section
...
- **`features`** — `cargo check --no-default-features --features X --tests`
  across fifteen feature sets: each of `native`, `tls`, `api`, `mcp`, `hep`,
  `metrics`, then `tls,api`, `native,tui,audio`, `native,tui,tls,hep,api`,
  `native,hep,api,mcp,mcp-http`, `bpf`, `vcon`, `audio`, `plugins`, and `wasm`
  (lib-only).

  `vcon`
  is in `full`, so `--all-features` already builds it. This leg is what proves
  it also builds without `tui`, `tls`, `api` or `mcp`.

  The documented
  headless server recipe (`native,hep,api,mcp,mcp-http`) gets a full `cargo
  test`, not just a compile check. Three legs also run the tests that exist
  only where a feature is absent, which no `--all-features` or `--features
  full` run can compile: `native` runs `feature_gate_test` (no `mcp`, no
  `hep`), `mcp` runs `app_servers_test` (no `mcp-http`), and
  `native,tui,tls,hep,api` runs `tui_snapshot_test` (no `audio`).

  `bpf` is here because every published
  ...
  actually built, under `SIPNAB_BPF_REQUIRED=1`.

  `--tests` is what makes any of
...
at `cargo publish`. [`tests/crate_package_test.rs`](../../tests/crate_package_test.rs) is the fast half: it
...
[`pre-commit`](../../.githooks/pre-commit) runs thirteen numbered gates,
...
its own rendering, because the three have different room to print in.
...
pages still read true, not a claim that they don't.

The gate that *does* fail
...
and a prose line above every one.

What it cannot catch is prose that has
...
[`pre-push`](../../.githooks/pre-push) picks both up before anything leaves the
...
The matrix used to be the exception: pre-push checked three of the fifteen
...
fifteen locally, reading both the combo list and `RUSTFLAGS` out of
...
[RFC 7950 section 11](https://www.rfc-editor.org/rfc/rfc7950#section-11).
...
pre-commit time the commit under judgement does not exist yet, so every one of
them reports "nothing past the tag" and passes without looking.

The hook also
sets `SIPNAB_RELEASE_GATES_STRICT=1`, which turns "git could not tell me" from
a skip into a failure. Skipping is right in a shallow clone and wrong here,
where git has everything it needs, and a gate that still cannot answer has a
bug rather than a limitation.

Rustdoc lints and the
...
CI does catch it: `quality.yml` runs `zola build`, and its
...
Zola publishes no aarch64 Linux binary, so on some
machines here the gate reports `NOT CHECKED` and names the
`cargo install --git` line that fixes it.

The prose pair arrived last and for cause: on
...
After those ten comes one gate that is not hard — the corpus gate
...
someone builds without that feature.

It has bitten twice — the `features` job
...
(behind `native`) took a whole test target out of every build without it.

Three
combinations, not fifteen: `tls` and `api` are the ones that *exclude* `native`,
...
non-Linux build in this project.

Two macOS breaks reached it hours apart on
...
[`scripts/check-non-linux.sh`](../../scripts/check-non-linux.sh) copies
...
a gap neither CI nor a Linux developer can reach.

Broken intra-doc links never
...
pass is newer than those figures and is not included in them.
...
is tight — it is disposable and the next run rebuilds it in 50 seconds.
...
100% counts as a critical failure.

Nothing enforced that rule between one manual
...
The hook greps the tree for that list instead of carrying its own copy,
...
the whole capture to `.git/sipnab-pre-push-corpus.log` and prints that path.
...
shared package-cache lock.

An optimized profile rather than dev: 8.8 GB of
...
accounting, and RTP-quality provenance.
...
That is a second variable rather than a reuse of
...
[`bpf/rust-toolchain.toml`](../../bpf/rust-toolchain.toml) is the one
...
link and version badge draws from.

They are different facts, and conflating
...
The same split applies to the docs:
...
fails the commit rather than shipping a documented `curl` that 404s.
...
resolution from choosing it, and nothing removes it.

So the job `needs: release`, and a tag
whose build fails never reaches crates.io. It runs
[`scripts/publish-crates.py`](../../scripts/publish-crates.py).
...
This is crates.io trusted publishing, and crates.io checks two things.
...
The environment must match too. The job runs in the `crates-io`
...
without anyone approving it: pushing the tag is the decision.
...
[`tests/crates_io_publish_test.rs`](../../tests/crates_io_publish_test.rs) holds
...
there shows up when crates.io refuses the next release's job.

To publish by hand, for
...
the next commit no suite runs, so nothing is watching that window.
...
pins describe the binaries rather than change them.
...
both — wrong for each, and concealing that they differ.

That gate also refuses
...
and `sipnab-audio-<version>.cdx.json` for the playback plugin.
...
`THIRD-PARTY-NOTICES.md` comes from that same set, named
...
`cargo-cyclonedx` has no `--package` flag: one workspace-level
...
[`scripts/split-debuginfo.sh`](../../scripts/split-debuginfo.sh) straight after
the build:

- **Linux.** The build appends `-C strip=none` to `RUSTFLAGS`, so the linker keeps
...
- **macOS.** The build appends `-C strip=none -C split-debuginfo=unpacked`,
  so the binary keeps its debug map and the DWARF stays in the object files
  it names. The script runs `dsymutil` itself, checks that the bundle's UUID
  is the binary's, zips it, strips the binary in place, and checks that the
  stripped binary kept its UUID and lost its debug map. It used to rely on
  rustc's `split-debuginfo=packed` leaving a `.dSYM` behind, and the first CI
  run of the macOS leg found none.
...
binary match the linker-stripped build byte for byte.

**Appended to `RUSTFLAGS`, never only `--config`.** A `RUSTFLAGS` variable set
...
see the file that ships. The size ceiling in [`website/config.toml`](../../website/config.toml) measures the
...
[`tests/split_debuginfo_test.rs`](../../tests/split_debuginfo_test.rs) drives
...
[`tests/release_debug_symbols_test.rs`](../../tests/release_debug_symbols_test.rs)
holds the workflow to the step order, the uploads and the checksums, and runs
the split step's own shell.

The release runs only on a tag, and the macOS half of the split runs on no
other job there, so a break in it would first show on a tag that is already
public. The `symbol-split` job in `ci.yml` closes that gap on every push. It
builds the release profile with the same `--rustflags` flags on
`x86_64-unknown-linux-gnu` and `aarch64-apple-darwin`, runs the split, and
checks the result with the platform's own tools: on Linux, that the `.debug`
build ID equals the binary's and that the binary has no `.symtab` and has a
`.gnu_debuglink`, and on macOS, that the `.dSYM.zip` exists and the bundle's
UUID from `dwarfdump --uuid` is the binary's. The release-workflow tests run
that step's own shell against real and deliberately broken splits.
