...
> sipnab's packets.
>
> `--cores` workers really do own their stores outright
...
> ([`batch.rs`](../../src/app/batch.rs)).
>
> There is no deadlock, because the
...
- **Disjoint guards** — [`process_packet()`](../../src/pipeline.rs) on the live
  ...
  TUI file-open path ([rule 1](#1-one-writer-per-store-per-run-mode)'s documented exception). Each locks a store once,
...
That is no longer so: findings, per-message output and
...
existed. That was a property of which tools somebody happened to write.
...
A rate limit survives the split in half because a
...
RTCP's length framing. `rtp::rtcp::looks_like_rtcp` then gained [RFC 3550](https://www.rfc-editor.org/rfc/rfc3550)
...
operators it applied to the classifier every media datagram reaches.
...
each map with unique keys and asserts the cap holds.

The dialog store offers
...
member it was writing when a ceiling hits.
...
A ZIP ([`zipped.rs`](../../src/capture/archive/zipped.rs), the `archive`
...
[`password.rs`](../../src/capture/archive/password.rs): `MAX_PASSWORD_BYTES`
...
[`sevenz.rs`](../../src/capture/archive/sevenz.rs) reports the refusal as
`bound_exceeded (7z key derivation)`. The REST wrong-password limiter
in [`api.rs`](../../src/output/api.rs) keys on a client and an archive
...
The per-source maps of the other four security detectors are
...
Admitting a key past the cap evicts the least recently used
...
A HEP listener's sender roster
([`hep_roster.rs`](../../src/capture/hep_roster.rs)) holds two such maps. The
sender table keys on the capture id and address a sender claims, holds at most
`[limits] max_tracked_peers` (default 4096), and REFUSES a new sender at the
bound rather than evicting one. It is also the per-sender frame ordinal table,
and a recycled counter would mint a second frame 0. The listener still admits
a refused sender's packets and counts them as untracked.

The refused-source
table keys on address alone, holds `HEP_REFUSED_SOURCES_TRACKED` (256), and
evicts the least recently refused address, so a spoofed flood rotates it. The
per-reason refusal totals live outside it, and nothing evicts them.
...
That last one is best-effort and says so: a low
...
[`ArchivePassword`](../../src/capture/archive/password.rs) holds bytes in a
...
never what it supplied. [`tests/archive_password_test.rs`](../../tests/archive_password_test.rs) runs every source at
...
a scrape-only credential.

Tests pin enforcement at all three layers —
...
`scope` means `full`, where an absent `aud` fails closed.
...
them would revoke live credentials on upgrade.
...
return later as evidence the agent then cites.

There is no `list_findings`, and
...
inside the deadline an MCP client gives one call.
...
timeout where they asked for a rate.

Three `const` assertions in
...
one that compiles to prove the type is reachable.

`NoteText::new` refuses an
...
line is at most `MAX_LINE_BYTES` long.

The unit tests in the same file pair
...
[`annotate_import_gate_test`](../../tests/annotate_import_gate_test.rs) fails
...
and [`src/capture/writer.rs`](../../src/capture/writer.rs). It names
`src/mcp/`, `src/output/`, `src/sip/`, `src/rtp/`, `src/security/` and
[`src/analysis.rs`](../../src/analysis.rs) as forbidden outright.
...
In the TUI the note pane and the editor draw the text from inside the
...
The architecture is still right for other reasons, and the reader
...
- The SIP-over-WebSocket skip tally repeated the `--portrange` defect above,
  ...
  produces, while an entire WebRTC signaling leg went unreported.
