...
**Affected code:** [`src/capture/hep.rs:788-889`](https://github.com/NormB/sipnab/blob/main/src/capture/hep.rs#L788-L889), [`src/capture/hep.rs:202-340`](https://github.com/NormB/sipnab/blob/main/src/capture/hep.rs#L202-L340), [`src/app/batch.rs:1264-1311`](https://github.com/NormB/sipnab/blob/main/src/app/batch.rs#L1264-L1311), [`src/process_isolation.rs:479-578`](https://github.com/NormB/sipnab/blob/main/src/process_isolation.rs#L479-L578)
...
- Receiver-side authentication added. `parse_hep_v3` now captures the `0x000e`
  ...
  and drops missing/invalid packets ([`src/capture/hep.rs`](https://github.com/NormB/sipnab/blob/main/src/capture/hep.rs)). The secret comes from
...
- HEP-origin packets are now flagged (`ParsedPacket.from_hep`, set in the
  ...
  both kill sites in [`src/app/batch.rs`](https://github.com/NormB/sipnab/blob/main/src/app/batch.rs); re-enabling requires the explicit
...
**Affected code:** [`src/output/prometheus_server.rs:28-151`](https://github.com/NormB/sipnab/blob/main/src/output/prometheus_server.rs#L28-L151), [`src/app/bootstrap.rs:214-223`](https://github.com/NormB/sipnab/blob/main/src/app/bootstrap.rs#L214-L223), [`src/app/tui_mode.rs:87-103`](https://github.com/NormB/sipnab/blob/main/src/app/tui_mode.rs#L87-L103), [`src/cli.rs:584-593`](https://github.com/NormB/sipnab/blob/main/src/cli.rs#L584-L593)
...
- `start_metrics_server` now refuses a non-loopback bind when `basic_auth` is
  ...
  ([`src/output/prometheus_server.rs`](https://github.com/NormB/sipnab/blob/main/src/output/prometheus_server.rs)).
...
**Affected code:** [`src/crash.rs:89-107`](https://github.com/NormB/sipnab/blob/main/src/crash.rs#L89-L107)
...
- `write_crash_report` now creates the file through `open_new_report_file`, which
  ...
  It retries with a fresh sequence suffix on collision ([`src/crash.rs`](https://github.com/NormB/sipnab/blob/main/src/crash.rs)).
...
- The audio plugin loader eventually attempts a bare platform filename after explicit and fixed paths ([`src/rtp/playback.rs:178-224`](https://github.com/NormB/sipnab/blob/main/src/rtp/playback.rs#L178-L224)). This broadens dynamic-library search behavior. The plugin is normally loaded after privilege drop, substantially limiting impact, but fixed absolute installation paths plus ownership/permission checks would make the trust claim enforceable. **Deferred:** left unchanged — the privilege-drop mitigation stands, and constraining the search path risks breaking platform-specific plugin discovery; tracked as a follow-up.
