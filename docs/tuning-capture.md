...
- **One run can hold several handles.** `--multi-device` opens one per named
  ...
  ([On a live device it means capture sockets, not workers](#on-a-live-device-it-means-capture-sockets-not-workers)). sipnab adds each handle's increment to one process-wide pair, so the
...
up the rest of a capture's quality — see [`GET /v1/stats`](rest-api.md#get-v1stats).
...
> at all. Read [Snapshot length](#4-snapshot-length---snaplen--and-why-it-decides-your-ring-capacity) and [Which interfaces: `any` versus named](#5-which-interfaces-any-versus-named) before concluding that a bigger `-B` did not help. The
...
- **Do not raise it** when the drops persist and a core is at 100%. A bigger ring
  ...
  and no buffer size fixes that. Go to [Capture less: BPF filters](#3-capture-less-bpf-filters).
...
The baseline is therefore signaling-only already, and this
...
clamp. **[Which interfaces: `any` versus named](#5-which-interfaces-any-versus-named) is the decision guide for the device half of that** — what leaving
...
2. **Name the interface** ([Which interfaces: `any` versus named](#5-which-interfaces-any-versus-named)) — the only way to reach `DLT_EN10MB` at all, and
   the prerequisite for the clamp below. It is a coverage trade, so read [Which interfaces: `any` versus named](#5-which-interfaces-any-versus-named)
   before making it.
3. **Turn the offloads off** ([Interface and driver drops](#7-interface-and-driver-drops)) — which is independently correct for capture
...
encapsulation or a fat SDP offer passes 400 bytes routinely.
...
The interactive 100 ms would have meant up to 100 ms before a
block retires. The batched path therefore uses its own
`BATCHED_READ_TIMEOUT_MS = 5` ([`src/capture/live.rs`](https://github.com/NormB/sipnab/blob/main/src/capture/live.rs)).
...
smaller, it comes back **wrong**, exactly as [Are you dropping packets?](#1-are-you-dropping-packets) describes for dropped packets.
...
TUI.** [Snapshot length](#4-snapshot-length---snaplen--and-why-it-decides-your-ring-capacity) has the mechanism and the run-mode rule that scopes it.
...
that clamp never runs.**

Every slot is the full 65535-byte snaplen regardless
...
`any` is one device, so it is one thread and one ring no
matter how many interfaces the traffic actually arrives on. `--cores N` ([On a live device it means capture sockets, not workers](#on-a-live-device-it-means-capture-sockets-not-workers)) is
...
- The drop counters from [Are you dropping packets?](#1-are-you-dropping-packets) read `no drops`. If it is not dropping, it is not
  costing you anything worth this trade.

**Name your interfaces** when any of these hold:

- [Are you dropping packets?](#1-are-you-dropping-packets) shows sustained `kernel buffer` drops.
...
nothing goes to `docker0`.

Neither half works alone — `any` cannot reach
the clamp however you set `ethtool`, and a named device with offloads on is
still stuck at ~65 KB slots ([Snapshot length](#4-snapshot-length---snaplen--and-why-it-decides-your-ring-capacity)).
...
costs `-B` **per device** ([The kernel capture buffer](#2-the-kernel-capture-buffer--b----buffer)), so eight interfaces at the default reserve half
...
decides whether it can, and that libpcap differs by artifact.

[Which capture
backends an artifact can
reach](install.md#which-capture-backends-an-artifact-can-reach) has the table,
and the second line of `sipnab --version` names the libpcap your binary
runs and the alternate backends its banner names.
...
Measured on the reference corpus ([benchmarks](benchmarks.md)): 1.13M pkts/s at
...
Reach for it when [Are you dropping packets?](#1-are-you-dropping-packets) shows sustained `kernel buffer` drops on ONE busy
interface and [The kernel capture buffer](#2-the-kernel-capture-buffer--b----buffer) through [Which interfaces: `any` versus named](#5-which-interfaces-any-versus-named) have not cleared them. Several interfaces is
`--multi-device` ([Which interfaces: `any` versus named](#5-which-interfaces-any-versus-named)) instead.
...
Note that this already makes the interface choice from [Which interfaces: `any` versus named](#5-which-interfaces-any-versus-named): it names `eth0` rather than taking
...
[The kernel capture buffer](#2-the-kernel-capture-buffer--b----buffer) → [Capture less: BPF filters](#3-capture-less-bpf-filters) → [Which interfaces: `any` versus named](#5-which-interfaces-any-versus-named) → [Give sipnab less work](#6-give-sipnab-less-work) → [Interface and driver drops](#7-interface-and-driver-drops) in that order, and re-measure after each change rather
...
- [CLI reference](cli-reference.md#capture) — every capture flag
- [Configuration](config-reference.md#capture) — the `[capture]` section
