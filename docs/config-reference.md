...
| Key | Type | Default | Description |
|-----|------|---------|-------------|
| `xcid_headers` | array | `[]` | Header names used to correlate B2BUA call legs (`sip.xcid`). A dialog whose message carries one of these headers pointing at another dialog's Call-ID joins that dialog. Add your estate's headers here. Empty by default: sipnab picks no `X-` name for you ([RFC 6648](https://www.rfc-editor.org/rfc/rfc6648)), and the identifier that crosses a B2BUA by design is [RFC 7989](https://www.rfc-editor.org/rfc/rfc7989) `Session-ID`, which correlates with no configuration. An empty list turns the header strategy off |
...
| `mcp_max_wait_seconds` | integer | `60` | Longest ONE `await_condition` MCP call may wait, in seconds. `mcp_max_rows` and `mcp_max_body_bytes` bound what an answer carries; this bounds how long a caller may hold one of `--mcp-max-concurrent` slots while carrying nothing. sipnab clamps a larger request to it and says so in the response. `--mcp-max-wait-seconds` overrides it. `0` fails validation and names the key |
...
| `quality_interval_secs` | integer | `5` | Seconds between RTP quality snapshots, which is the resolution of the per-stream quality trend on `rtp_stats`, `GET /v1/streams/{id}`, the TUI and the browser analyzer. The shipped five seconds averages away any burst shorter than itself. The trend still covers an hour of call time whatever this is, so a one-second setting retains 3600 snapshots per stream rather than 720 — resolution costs memory, never history. `--quality-interval` overrides it. Anything outside 1-300 fails validation and names the key |
