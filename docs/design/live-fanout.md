...
**Status:** DESIGN. Nothing here is implemented. Section 7 recommends
...
in full — [`capture-tuning-tasks.md:22`](capture-tuning-tasks.md): *"Nothing on
...
| Piece | Where | State |
|---|---|---|
| `setsockopt(SOL_PACKET, PACKET_FANOUT, …)` | [`fanout.rs:76`](../../src/capture/fanout.rs) | written, unit-tested, and **verified against the running kernel** by an `#[ignore]`d root test |
| Platform split | [`fanout.rs:101`](../../src/capture/fanout.rs) | non-Linux returns `ErrorKind::Unsupported`; the call site is unconditional so it cannot go unused (`82eb8ff`) |
| Plan / group id | [`live.rs:194`](../../src/capture/live.rs), [`:209`](../../src/capture/live.rs) | pure, tested without a device |
| Kernel probe | [`live.rs:293`](../../src/capture/live.rs) | throwaway handle, so refusal is discovered once |
| N-socket driver | `capture_live_fanout`, [`live.rs:224`](../../src/capture/live.rs) | complete, joins all threads, first error wins |
| **A caller** | — | **none** |
...
`capture_live` ([`native.rs:256-262`](../../src/capture/native.rs)), and
`capture_live` is a one-line wrapper that passes `None` for the group
([`live.rs:175`](../../src/capture/live.rs)).
...
- The help text, [`cli.rs:1447-1453`](../../src/cli.rs): *"CPU cores for OFFLINE
  ...
  path regardless."*
- `cores_ignored_warning` ([`bootstrap.rs:1875`](../../src/app/bootstrap.rs)),
...
- `cores_warning_is_the_exact_complement_of_the_parallel_path`
  ([`bootstrap.rs:2496`](../../src/app/bootstrap.rs)), which asserts the warning
...
[`parallel.rs:136-139`](../../src/parallel.rs)). Live, as built, it would buy N
...
([`live.rs:236`](../../src/capture/live.rs)): *"`--cores {sockets}` does not
...
2. **`cores_ignored_warning` loses its live branch and keeps its
   ...
   ([`bootstrap.rs:1880`](../../src/app/bootstrap.rs)) stays true; see §2.1.
...
3. **The run says what it bought.** `capture_live_fanout` already logs
   *"capturing on {sockets} sockets, fanout group {group}"*
   ([`live.rs:256`](../../src/capture/live.rs)). That line must also say that
...
([`bootstrap.rs:519`](../../src/app/bootstrap.rs)) stays exactly as it is, still
...
(`DEFAULT_BUFFER_MB`, [`native.rs:127`](../../src/capture/native.rs)). So
...
buffer ladder ([`live.rs:341`](../../src/capture/live.rs)) will step each
...
`start_multi_capture` ([`native.rs:352`](../../src/capture/native.rs)) already
...
([`live.rs:209`](../../src/capture/live.rs)) derives **one group id per
...
([`fanout.rs:183`](../../src/capture/fanout.rs)) already set for the single-device
...
([`live.rs:261`](../../src/capture/live.rs)), and there is one receiver: the
`rx.recv_timeout` at [`batch.rs:2121`](../../src/app/batch.rs). So the shape is
...
([`channel.rs:131`](../../src/capture/channel.rs)) is an unbounded data queue
...
| `interface_dropped` rising | neither; the NIC discarded before libpcap, and no userspace change recovers it ([`server.rs:1719`](../../src/mcp/server.rs)) |

`sipnab_capture_queue_depth_packets` and
`sipnab_capture_backpressure_blocks_total`
([`prometheus.rs:470`](../../src/output/prometheus.rs),
[`:482`](../../src/output/prometheus.rs)) are the second and third rows'
instrument, fed from `CaptureMeter`
([`channel.rs:68-86`](../../src/capture/channel.rs)), and they are scrapeable
from a headless run — `start_servers` is called with `metrics: true` and the
real meter from [`batch.rs:1861-1879`](../../src/app/batch.rs).

**One gap worth fixing before the experiment.** `CaptureCounters`, the
`capture_health` MCP response ([`server.rs:1712`](../../src/mcp/server.rs)),
...
[`fanout.rs:31-35`](../../src/capture/fanout.rs) already says this, and
...
1. **`DialogStore::merge` reassembles proxied calls.** A call through a proxy is
   ...
   ([`parallel.rs:18-30`](../../src/parallel.rs)) — *"in one 100 MB file of the
...
2. **`StreamStore::reassociate_all` links streams to dialogs globally**
   ([`stream_store.rs:1201`](../../src/rtp/stream_store.rs)), because SDP and RTP
...
3. **`final_sweep` runs exactly ONCE, after the merge, at the capture's final
   timestamp** ([`parallel.rs:116`](../../src/parallel.rs)) — and
   [`parallel.rs:47-53`](../../src/parallel.rs) records why it is not per worker:
...
([`batch.rs:2083-2093`](../../src/app/batch.rs)) against one `SweepClock` fed by
...
([`live.rs:714`](../../src/capture/live.rs)) are the loss counters;
`sipnab_capture_queue_depth_packets` and
...
- **Drop-instrument calibration.** The top rung is run once with a 2 MiB ring and
  **both** `_quality_degraded == 1` and `kernel_dropped > 0` are required,
  ...
  ([`live.rs:523`](../../src/capture/live.rs)) while the default level is `info`
...
([`fanout.rs:183`](../../src/capture/fanout.rs)); this one has not been, and
...
- **Does `ROLLOVER` change the answer?** `FANOUT_HASH_WITH_ROLLOVER`
  ([`fanout.rs:47`](../../src/capture/fanout.rs)) lets a full ring spill to
...
- **What does `PACKET_FANOUT` do on the `any` pseudo-device?**
  ...
  ([`fanout.rs:67-68`](../../src/capture/fanout.rs)) but marks it "on some
  kernels". The zero-argument default sniffs all interfaces via `any`
  ([`cli.rs:348-349`](../../src/cli.rs)), so this is the *default* invocation,
  not an edge case. The probe at [`live.rs:293`](../../src/capture/live.rs) will
...
- **Is `immediate_mode` right for N sockets?** `immediate_mode_for`
  ([`bootstrap.rs:1537`](../../src/app/bootstrap.rs)) returns true only for the
