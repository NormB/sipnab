...
18 fuzz targets under [`fuzz/fuzz_targets/`](https://github.com/NormB/sipnab/tree/main/fuzz/fuzz_targets) — `sip_parser`, `sdp_parser`,
...
`keylog_line`, `filter_dsl`, `rtpengine_ng`, `stun` and `llmnr` — with `fuzz-check` keeping them compiling in CI
