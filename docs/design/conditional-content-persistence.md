...
- **Feature gate:** every item lives behind `#[cfg(feature = "vcon")]`.
  ...
  [`src/app/batch.rs:5132`](https://github.com/NormB/sipnab/blob/main/src/app/batch.rs#L5132), and a new entry point needs the same pairing or the
...
- Modify: [`src/app/batch.rs:5042`](https://github.com/NormB/sipnab/blob/main/src/app/batch.rs#L5042) (`export_vcon`)
...
Extend `export_vcon` at [`src/app/batch.rs:5042`](https://github.com/NormB/sipnab/blob/main/src/app/batch.rs#L5042): when `export_vcon_when` is set, loop the selected Call-IDs, build each container with the existing single-call path, and write it to `<dir>/<sanitized-call-id>.vcon.json`. Sanitize by replacing every character outside `[A-Za-z0-9._-]` with `_`, because a Call-ID is attacker-influenced text and reaches a filesystem path here.
