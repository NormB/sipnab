...
workers free it on another thread. This is the largest driver in the profile.
...
Expected: the largest single win available. Verify by re-profiling and watching
`mi_free`, `mi_abandoned_page_try_reclaim` and `_mi_page_malloc_zero`.
