...
|---|---|---|---|
| Canonical | [`src/pipeline.rs:2405`](https://github.com/NormB/sipnab/blob/main/src/pipeline.rs#L2405) `process_packet` | 142 ln | TUI live |
...
| MCP | [`src/mcp/server.rs:8979`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L8979) `DialogSummary` | **`message_count`**, **`format!("{:?}", method)`** |
...
3. **Property-based testing.** proptest as dev-dep; start with three
   ...
   "parsed correctly").
4. **Latest-stable CI leg.** Everything pins 1.98.1 (good as an MSRV gate);
