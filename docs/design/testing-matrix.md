...
| `--portrange` |  | `RANGE` | Capture | e2e | `tests/analyze_test.rs`, `tests/config_wiring_test.rs` +4 |  |  |
...
| `--notes` |  | `FILE` | Operator notes | referenced | `src/app/bootstrap.rs`, `tests/annotate_cli_test.rs` |  |  |
...
| `--hep-listen` | `-L` | `ADDR` | HEP | e2e | `src/app/bootstrap.rs`, `tests/cli_flag_behavior_test.rs` +6 |  |  |
