...
| `--input` | `-I` |  | Capture | e2e | `src/app/tui_mode.rs`, `tests/accused_sources_test.rs` +75 |  |  |
...
| `--portrange` |  | `RANGE` | Capture | e2e | `tests/accused_sources_test.rs`, `tests/analyze_test.rs` +11 |  |  |
...
| `--no-tui` | `-N` |  | Mode | e2e | `tests/accused_sources_test.rs`, `tests/analyze_test.rs` +76 |  |  |
| `--calls-only` | `-c` |  | Mode | e2e | `tests/cli_flag_behavior_test.rs`, `tests/cli_options_test.rs` +1 |  |  |
| `--telephone-event` | `-t` |  | Mode | e2e | `tests/cli_options_test.rs`, `tests/dtmf_masking_test.rs` +2 |  |  |
| `--dtmf-cleartext` |  |  | Mode | e2e | `tests/dtmf_masking_test.rs` | **behavior** | dtmf_cleartext_emits_the_digit_value_at_debug_level (tests/dtmf_masking_test.rs), with an anti-vacuity guard |
| `--quiet` | `-q` |  | Mode | e2e | `tests/app_servers_wiring_test.rs`, `tests/capture_clock_test.rs` +34 |  |  |
...
| `--invert` | `-v` |  | Matching | e2e | `tests/cli_flag_behavior_test.rs`, `tests/cli_options_test.rs` |  |  |
| `--word` | `-w` |  | Matching | e2e | `tests/cli_flag_behavior_test.rs`, `tests/cli_options_test.rs` |  |  |
| `--single-line` |  |  | Matching | e2e | `tests/cli_options_test.rs` | **behavior** | single_line_stops_dot_matching_newline (tests/cli_options_test.rs): 7 messages without, 0 with |
| `--from` |  | `PATTERN` | Matching | e2e | `tests/cli_flag_behavior_test.rs`, `tests/cli_options_test.rs` +2 |  |  |
| `--to` |  | `PATTERN` | Matching | e2e | `tests/cli_options_test.rs`, `tests/cli_test.rs` |  |  |
| `--contact` |  | `PATTERN` | Matching | e2e | `tests/cli_options_test.rs` |  |  |
| `--ua` |  | `PATTERN` | Matching | e2e | `tests/cli_flag_behavior_test.rs`, `tests/cli_options_test.rs` |  |  |
| `--filter` |  | `EXPR` | Matching | e2e | `tests/analyze_test.rs`, `tests/cli_options_test.rs` +5 |  |  |
| `--problems` |  |  | Diagnostic aliases | e2e | `tests/cli_options_test.rs`, `tests/cli_test.rs` +3 |  |  |
| `--slow-setup` |  |  | Diagnostic aliases | e2e | `tests/cli_options_test.rs`, `tests/filter_corpus_test.rs` +1 | **behavior** | slow_setup_filter (tests/cli_options_test.rs); one-sided -- nothing positively selects a slow-setup dialog via the flag |
...
| `--json` |  |  | Output | e2e | `tests/annotate_cli_test.rs`, `tests/cli_flag_behavior_test.rs` +14 |  |  |
| `--json-pretty` |  |  | Output | e2e | `tests/cli_options_test.rs`, `tests/json_schema_test.rs` +1 |  |  |
| `--json-dialogs` |  |  | Output | e2e | `tests/cli_flag_behavior_test.rs`, `tests/config_wiring_test.rs` +20 |  |  |
| `--plugin` |  | `PATH` | Output | e2e | `tests/partial_run_exit_code_test.rs`, `tests/plugin_example_test.rs` |  |  |
| `--report` |  |  | Output | e2e | `tests/capture_clock_test.rs`, `tests/cli_flag_behavior_test.rs` +17 |  |  |
...
| `--line-buffer` |  |  | Output | e2e | `tests/cli_options_test.rs` | **parse-only** | line_buffer_flag asserts "still emits all 7"; the flush effect is tested on the sink parameter, never joined to the flag |
...
| `--no-cli-print` |  |  | Output | e2e | `tests/analyze_test.rs`, `tests/batch_run_paths_test.rs` +30 |  |  |
...
| `--lint` |  |  | Output | e2e | `tests/cli_options_test.rs`, `tests/config_wiring_test.rs` +1 |  |  |
...
| `--limit` | `-l` | `N` | Dialog | e2e | `src/app/tui_mode.rs`, `tests/cli_flag_behavior_test.rs` +4 |  |  |
...
| `--max-streams` |  | `N` | RTP | e2e | `tests/cli_options_test.rs`, `tests/config_wiring_test.rs` |  |  |
...
| `--kill-response` |  | `CODE` | Security | e2e | `tests/cli_options_test.rs`, `tests/config_wiring_test.rs` +1 |  |  |
...
| `--fraud-wangiri-calls` |  | `N` | Security | e2e | `tests/threshold_wiring_test.rs` | **behavior** | fraud_wangiri_calls_decides_how_many_short_calls_are_a_lure |
| `--fraud-sequential-calls` |  | `N` | Security | e2e | `tests/threshold_wiring_test.rs` | **behavior** | fraud_sequential_calls_decides_how_long_a_scan_must_be |
| `--fraud-volume-multiplier` |  | `N` | Security | e2e | `tests/threshold_wiring_test.rs` | **behavior** | fraud_volume_multiplier_decides_how_big_a_spike_must_be |
| `--fraud-volume-min-calls` |  | `N` | Security | e2e | `tests/threshold_wiring_test.rs` | **behavior** | fraud_volume_min_calls_decides_the_floor_under_a_spike |
| `--fraud-volume-window` |  | `SECS` | Security | e2e | `tests/threshold_wiring_test.rs` | **behavior** | fraud_volume_window_secs_decides_how_concentrated_a_burst_must_be |
| `--fraud-wangiri-window` |  | `SECS` | Security | e2e | `tests/threshold_wiring_test.rs` | **behavior** | fraud_wangiri_window_secs_decides_whether_a_paced_lure_is_visible |
| `--scanner-behavioral-probes` |  | `N` | Security | e2e | `tests/threshold_wiring_test.rs` | **behavior** | scanner_behavioral_probes_decides_when_an_aggregated_rate_is_a_scanner |
| `--scanner-enumeration-targets` |  | `N` | Security | e2e | `tests/threshold_wiring_test.rs` | **behavior** | scanner_enumeration_targets_decides_how_wide_a_sweep_must_be |
| `--scanner-rejected-probes` |  | `N` | Security | e2e | `tests/threshold_wiring_test.rs` | **behavior** | scanner_rejected_probes_decides_how_much_refusal_is_evidence |
| `--scanner-unanswered-probes` |  | `N` | Security | e2e | `tests/threshold_wiring_test.rs` | **behavior** | scanner_unanswered_probes_decides_how_much_silence_is_evidence |
| `--scanner-window` |  | `SECS` | Security | e2e | `tests/threshold_wiring_test.rs` | **behavior** | scanner_window_secs_decides_whether_a_paced_sweep_is_visible |
| `--scanner-established-factor` |  | `N` | Security | e2e | `tests/threshold_wiring_test.rs` | **behavior** | scanner_established_factor_decides_what_a_registration_buys |
| `--scanner-answer-grace` |  | `MS` | Security | e2e | `tests/threshold_wiring_test.rs` | **behavior** | scanner_answer_grace_ms_decides_how_slow_a_link_may_be |
| `--findings-history` |  | `N` | Security | parsed | `src/cli.rs` |  |  |
| `--digest-leak` |  |  | Security | e2e | `src/app/tui_mode.rs`, `tests/cli_options_test.rs` | **parse-only** | INTRUSION DETECTOR. The only tests assert the flag changes nothing on a clean fixture; a regression that stops building the detector passes them |
...
| `--pdd-threshold` |  | `SECS` | Analysis | e2e | `tests/metrics_headless_test.rs`, `tests/threshold_wiring_test.rs` |  |  |
| `--ack-timeout` |  | `SECS` | Analysis | e2e | `tests/threshold_wiring_test.rs` | **behavior** | diagnosis_ack_timeout_decides_when_a_missing_ack_is_a_fault; flag passed to a subprocess, NDJSON changes |
| `--no-final-response-timeout` |  | `SECS` | Analysis | e2e | `tests/threshold_wiring_test.rs` | **behavior** | diagnosis_no_final_response_timeout_decides_when_silence_is_reported |
| `--duration-asymmetry-pct` |  | `PCT` | Analysis | e2e | `tests/threshold_wiring_test.rs` | **behavior** | diagnosis_duration_asymmetry_percentage_reaches_the_diagnosis |
| `--duration-asymmetry-secs` |  | `SECS` | Analysis | e2e | `tests/threshold_wiring_test.rs` | **behavior** | diagnosis_duration_asymmetry_threshold_decides_when_legs_disagree |
| `--late-media-ms` |  | `MS` | Analysis | e2e | `tests/threshold_wiring_test.rs` | **behavior** | diagnosis_late_media_threshold_decides_when_media_is_late |
| `--cn-suppression-ratio` |  | `RATIO` | Analysis | e2e | `tests/threshold_wiring_test.rs` |  |  |
...
| `--keylog` |  | `FILE` | TLS / Decryption | e2e | `tests/batch_run_paths_test.rs`, `tests/doc_commands_run_test.rs` |  |  |
...
| `--allow-coredump` |  |  | TLS / Decryption | e2e | `tests/cli_options_test.rs` | **parse-only** | SAFETY SWITCH. Its smoke test runs on a capture with NO key material, so the code it guards never executes either way |
...
| `--cores` |  | `N` | Resource limits | e2e | `tests/batch_run_paths_test.rs`, `tests/capture_clock_test.rs` +7 |  |  |
...
| `--no-config` | `-F` |  | Config | e2e | `tests/annotate_cli_test.rs`, `tests/cli_options_test.rs` +8 |  |  |
