...
- [x] **LK1 — `fork`/`exec`, stdout writes and a third lock all happen while
  ...
  nowhere; `security_findings` ([`src/mcp/server.rs:2991`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L2991)) currently takes
...
- [x] **G1 — `INVALID_PCAP_TIMESTAMPS` is counted and warned but never
  ...
  it); the MCP `capture_status` tool carries the field ([`src/mcp/server.rs:3042`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L3042),
...
- [ ] **PA1 — Packet-level provenance (`_ref` + `show_evidence`).** Every fact
  ...
  - **In progress — the resolver end exists; the threading is partial (status
    ...
    (`#[tool(` at [`src/mcp/server.rs:4037`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L4037), handler at `:3866`), confined to
    ...
    `findings_with_refs` ([`src/mcp/server.rs:1005`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L1005)), which attaches `frame_ref`
    to `lint_dialog`
...
- [ ] **PA4 — Complete the linter rule corpus.** The engine and the
  ...
  - **`.sipnablint` is half wired. Corrected 2026-08-05:** this used to read
    *"`LintConfig::suppress_list` parses the file shape and nothing loads a file
    ...
    ([`src/mcp/server.rs:480`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L480)), which takes an explicit filename or walks up from
...
- [x] **PB9 — MCP token scoping.** REST has `--token-scope full|metrics`;
  ...
  **Done, and this entry read as unstarted-but-unblocked until 2026-08-06.**
  ...
  `scope_of` ([`src/mcp/server.rs:5101`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L5101), the `mcp-http` arm), reading the scope out of the
