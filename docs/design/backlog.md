...
- [x] **G6 — `--cores N` is silently ignored on live capture.** `RunMode` is
  ...
  ([`src/app/bootstrap.rs:3388`](https://github.com/NormB/sipnab/blob/main/src/app/bootstrap.rs#L3388)) returns the message and the reason —
...
- [x] **CT3 — `--snaplen` defaults to 65535, so every packet is copied whole
  ...
  until 2026-08-06.** `snaplen_truncation_warning` ([`src/app/bootstrap.rs:3595`](https://github.com/NormB/sipnab/blob/main/src/app/bootstrap.rs#L3595),
