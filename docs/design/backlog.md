...
- [x] **LK1 — `fork`/`exec`, stdout writes and a third lock all happen while
  ...
  nowhere; `security_findings` ([`src/mcp/server.rs:4106`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L4106)) currently takes
  nowhere; `security_findings` ([`src/mcp/server.rs:4106`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L4106)) currently takes
...
- [x] **G1 — `INVALID_PCAP_TIMESTAMPS` is counted and warned but never
  ...
  it); the MCP `capture_status` tool carries the field ([`src/mcp/server.rs:4203`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L4203),
  it); the MCP `capture_status` tool carries the field ([`src/mcp/server.rs:4203`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L4203),
...
- [ ] **PA1 — Packet-level provenance (`_ref` + `show_evidence`).** Every fact
  ...
  - **In progress — the resolver end exists; the threading is partial (status
    ...
    (`#[tool(` at [`src/mcp/server.rs:5510`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L5510), handler at `:3866`), confined to
    ...
    `findings_with_refs` ([`src/mcp/server.rs:1264`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L1264)), which attaches `frame_ref`
...
- [x] **PB9 — MCP token scoping.** REST has `--token-scope full|metrics`;
  ...
  **Done, and this entry read as unstarted-but-unblocked until 2026-08-06.**
  ...
  `scope_of` ([`src/mcp/server.rs:6599`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L6599), the `mcp-http` arm), reading the scope out of the
