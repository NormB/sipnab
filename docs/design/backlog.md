...
- [x] **LK1 — `fork`/`exec`, stdout writes and a third lock all happen while
  ...
  nowhere; `security_findings` ([`src/mcp/server.rs:3374`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L3374)) currently takes
...
- [x] **G1 — `INVALID_PCAP_TIMESTAMPS` is counted and warned but never
  ...
  it); the MCP `capture_status` tool carries the field ([`src/mcp/server.rs:3471`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L3471),
  ...
  **Corrected 2026-08-06:** a fourth counter, `undecodable_frames`, has since
  ...
  `degraded` ([`src/output/prometheus.rs:327`](https://github.com/NormB/sipnab/blob/main/src/output/prometheus.rs#L327) lists what the flag actually
...
- [x] **PB9 — MCP token scoping.** REST has `--token-scope full|metrics`;
  ...
  **Done, and this entry read as unstarted-but-unblocked until 2026-08-06.**
  ...
  `scope_of` ([`src/mcp/server.rs:5592`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L5592), the `mcp-http` arm), reading the scope out of the
