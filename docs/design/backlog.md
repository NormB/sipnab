...
- [x] **G6 — `--cores N` is silently ignored on live capture.** `RunMode` is
  ...
  ([`src/app/bootstrap.rs:2293`](https://github.com/NormB/sipnab/blob/main/src/app/bootstrap.rs#L2293)) returns the message and the reason —
...
- [x] **SCTP transport parsing** — **Done:** `parse_packet` now decodes the SCTP
  ...
  **Corrected 2026-08-06:** this used to end *"multi-packet fragment reassembly
  ...
  `PacketProcessor` ([`src/capture/mod.rs:653`](https://github.com/NormB/sipnab/blob/main/src/capture/mod.rs#L653), `:651`, `:686`). The P2 entry
