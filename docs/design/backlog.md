...
- [x] **G6 — `--cores N` is silently ignored on live capture.** `RunMode` is
  ...
  ([`src/app/bootstrap.rs:3411`](https://github.com/NormB/sipnab/blob/main/src/app/bootstrap.rs#L3411)) returns the message and the reason —
...
- [x] **CT3 — `--snaplen` defaults to 65535, so every packet is copied whole
  ...
  until 2026-08-06.** `snaplen_truncation_warning` ([`src/app/bootstrap.rs:3618`](https://github.com/NormB/sipnab/blob/main/src/app/bootstrap.rs#L3618),
...
- [x] **FLT1 (done 2026-09-02) — `--export-vcon-when` refuses the aliases `--filter` accepts.**
  ...
  ([`src/app/batch.rs:5779`](https://github.com/NormB/sipnab/blob/main/src/app/batch.rs#L5779)) parses raw. The doc claim is corrected;
