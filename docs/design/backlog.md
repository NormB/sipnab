...
<!-- BACKLOG-STATUS: generated by scripts/backlog-status.py -- do not edit -->
...
- [x] **LK1 — `fork`/`exec`, stdout writes and a third lock all happen while
  ...
  nowhere; `security_findings` ([`src/mcp/server.rs:5570`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L5570)) currently takes
  nowhere; `security_findings` ([`src/mcp/server.rs:5570`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L5570)) currently takes
...
- [x] **VAL15 (done 2026-08-31) — `get_capture_report.frames_read` is latched to the capture the
...
- [x] **VAL18 (done 2026-08-31) — `search_messages` cannot see what the filter DSL can, and
...
- [x] **G1 — `INVALID_PCAP_TIMESTAMPS` is counted and warned but never
  ...
  it); the MCP `capture_status` tool carries the field ([`src/mcp/server.rs:5685`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L5685),
  it); the MCP `capture_status` tool carries the field ([`src/mcp/server.rs:5685`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L5685),
...
- [x] **PA1 — Packet-level provenance. DONE 2026-09-11.**
  ...
  **Spans threaded through the parser: DONE 2026-09-11.** This entry claimed
  "the parser already carries spans", and that was true of the BODY only.
  `SipHeader` now carries `line_span`, the byte range of its own logical line,
  recorded by `parse_headers_and_body` as it walks. The second walk of the
  header grammar is gone, and with it the whole class of drift it was guarded
  against: a line with no colon used to make the two walks disagree and cost
  EVERY header its range, because citing a neighbor is worse than citing
  nothing. One walk cannot disagree with itself, so a dropped line now costs
  nothing. `ranges_unavailable` survives with honest new meaning -- it counts
  headers that came from no bytes, synthesized or rewritten after the parse,
  which get no range rather than a plausible one. A folded header spans every
  ...
  **The rest of the composite encoding, and the per-call opt-out: NOT BUILT,
  ...
  - **In progress — the resolver end exists; the threading is partial (status
    ...
    (`#[tool(` at [`src/mcp/server.rs:7009`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L7009), handler at `:3866`), confined to
    ...
    `findings_with_refs` ([`src/mcp/server.rs:1809`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L1809)), which attaches `frame_ref`
...
- [x] **PB9 — MCP token scoping.** REST has `--token-scope full|metrics`;
  ...
  **Done, and this entry read as unstarted-but-unblocked until 2026-08-06.**
  ...
  `scope_of` ([`src/mcp/server.rs:8463`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L8463), the `mcp-http` arm), reading the scope out of the
...
2. `TK6`'s `ebpf` feature sits **outside `full`**, and
   ...
   ([`tests/site_journey_test.rs:6703`](https://github.com/NormB/sipnab/blob/main/tests/site_journey_test.rs#L6703)) fails on any `#[test]` or
...
- [x] **AS3 (done 2026-09-08) — termination cause as a first-class field.** `triage_call` returns
  ...
  such frames plus [RFC 3326](https://www.rfc-editor.org/rfc/rfc3326) `Reason: Q.850;cause=` values.
...
- [x] **DOC1 (done 2026-08-30) — [`docs/rest-api.md:832`](https://github.com/NormB/sipnab/blob/main/docs/rest-api.md#L832) promises the vCon endpoint returns no
...
- [x] **DOC2 (done 2026-08-30) — `--user` is a silent no-op unless the process is root, and the
...
- [x] **DOC3 (done 2026-08-30) — two pages disagree about whether MCP fences untrusted message
...
- [x] **DOC4 (done 2026-08-30) — [`docs/mcp-deploy.md:248`](https://github.com/NormB/sipnab/blob/main/docs/mcp-deploy.md#L248) opens the remote-access section by
  ...
  `ss.clear()` ([`src/mcp/server.rs:7527`](https://github.com/NormB/sipnab/blob/main/src/mcp/server.rs#L7527)). The code already knows: a note at
...
- [x] **DOC5 (done 2026-08-30) — the program tells operators to use flags that do not exist.**
  ...
  `--limit`, `--max-dialogs` or `--max-streams`"*. `--max-dialogs` is not a flag; the dialog cap is `--limit`, else
  `[limits] dialog_limit`. **This entry also claimed `--max-streams` is not a
  flag. It is** — measured against `--help` and by invocation on 2026-08-30 —
  so the messages now name `--limit` and `--max-streams`, and `--export-audio`
  became the `export_audio` MCP tool, which is what actually writes the WAV. This
...
- [x] **DOC6 (done 2026-08-30) — the HEP HMAC token version went 1 to 2 with no changelog entry
...
- [x] **DOC7 (done 2026-08-30) — the filter node cap is not in "Parser constraints".**
...
- [x] **DOC8 (done 2026-08-30) — exec hooks inherit every credential in the environment, and 8 of
...
- [x] **DOC9 (done 2026-08-30) — smaller contradictions, each doc:line against source:line.**
...
- [x] **DOC10 (done 2026-08-30) — nothing gates environment variables.** 34 read, 19 documented.
  The highest-yield missing gate, and `flag_coverage_test`'s ratchet shape
  transfers directly.
- [x] **DOC11 (done 2026-08-30) — nothing gates numeric policy ceilings.** `MAX_EXPRESSION_NODES`,
...
- [x] **DOC12 (done 2026-08-30) — the MCP docs gate reads one file.** `docs_drift_test.rs:3627`
