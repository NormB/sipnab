...
- [ ] **Audit the double-parse in batch+API mode.** Verified call chain today: `processor.process()` ([`src/app/batch.rs:2196`](https://github.com/NormB/sipnab/blob/main/src/app/batch.rs#L2196)) does Ethernet/IP/TCP/UDP reassembly only — no SIP parsing. `process_parsed_packet()` ([`src/app/batch.rs:4226`](https://github.com/NormB/sipnab/blob/main/src/app/batch.rs#L4226)) does the first SIP parse + `dialog_store.process_message` against the local store. `mirror_to_shared_stores()` (`:1326` invocation, `:2142` definition) does a second full SIP parse + a second `dialog_store.process_message` against the `Arc<RwLock<...>>` shared store. Result: every matching packet is parsed twice when `--api` is on in batch mode.
...
| `rtp_stats` | `StreamStore::iter` ([`src/rtp/stream_store.rs:1795`](https://github.com/NormB/sipnab/blob/main/src/rtp/stream_store.rs#L1795)) + `rtp::diagnosis::diagnose_media` + `output::json::stream_to_json` |
...
| `stats` | Mirrors `GET /v1/stats` from `output::api::get_stats` ([`src/output/api.rs:3680`](https://github.com/NormB/sipnab/blob/main/src/output/api.rs#L3680)) |

| Phase 8 infra | Reuses |
|---|---|
| Bind address parsing | `output::api::parse_bind_addr` ([`src/output/api.rs:736`](https://github.com/NormB/sipnab/blob/main/src/output/api.rs#L736)) |
...
| Rate limiting | `output::api::RateLimiter` ([`src/output/api.rs:335`](https://github.com/NormB/sipnab/blob/main/src/output/api.rs#L335)) |
...
| WebSocket / SSE Router mounting | Extend `output::api::build_router` ([`src/output/api.rs:658`](https://github.com/NormB/sipnab/blob/main/src/output/api.rs#L658)) with new routes; reuse the existing `guard()` middleware |
...
| OTel span on dialog state | `#[tracing::instrument]` on `DialogStore::process_message` ([`src/sip/dialog_store.rs:903`](https://github.com/NormB/sipnab/blob/main/src/sip/dialog_store.rs#L903)) |
...
| `ptime_asymmetry` | Inferred from RTP inter-arrival in `RtpStream::update` ([`src/rtp/stream.rs:1041`](https://github.com/NormB/sipnab/blob/main/src/rtp/stream.rs#L1041)) or SDP `a=ptime:` parsed in [`src/sip/sdp.rs`](https://github.com/NormB/sipnab/blob/main/src/sip/sdp.rs) |
...
| Six new diagnostic aliases | Extend `expand_alias` in [`src/sip/dsl.rs:510`](https://github.com/NormB/sipnab/blob/main/src/sip/dsl.rs#L510) |
