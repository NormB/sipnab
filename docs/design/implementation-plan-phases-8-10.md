...
- [ ] **Audit the double-parse in batch+API mode.** Verified call chain today: `processor.process()` ([`src/app/batch.rs:2196`](https://github.com/NormB/sipnab/blob/main/src/app/batch.rs#L2196)) does Ethernet/IP/TCP/UDP reassembly only — no SIP parsing. `process_parsed_packet()` ([`src/app/batch.rs:4385`](https://github.com/NormB/sipnab/blob/main/src/app/batch.rs#L4385)) does the first SIP parse + `dialog_store.process_message` against the local store. `mirror_to_shared_stores()` (`:1326` invocation, `:2142` definition) does a second full SIP parse + a second `dialog_store.process_message` against the `Arc<RwLock<...>>` shared store. Result: every matching packet is parsed twice when `--api` is on in batch mode.
...
| MCP tool | Wraps |
|---|---|
| `list_dialogs` | `DialogStore::iter` ([`src/sip/dialog_store.rs:1101`](https://github.com/NormB/sipnab/blob/main/src/sip/dialog_store.rs#L1101)) + `FilterExpr::matches_dialog` ([`src/sip/dsl.rs:798`](https://github.com/NormB/sipnab/blob/main/src/sip/dsl.rs#L798)) + `expand_alias` ([`src/sip/dsl.rs:510`](https://github.com/NormB/sipnab/blob/main/src/sip/dsl.rs#L510)) |
