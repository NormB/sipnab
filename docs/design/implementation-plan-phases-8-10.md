...
- [ ] **Audit the double-parse in batch+API mode.** Verified call chain today: `processor.process()` ([`src/app/batch.rs:2196`](https://github.com/NormB/sipnab/blob/main/src/app/batch.rs#L2196)) does Ethernet/IP/TCP/UDP reassembly only — no SIP parsing. `process_parsed_packet()` ([`src/app/batch.rs:3960`](https://github.com/NormB/sipnab/blob/main/src/app/batch.rs#L3960)) does the first SIP parse + `dialog_store.process_message` against the local store. `mirror_to_shared_stores()` (`:1326` invocation, `:2142` definition) does a second full SIP parse + a second `dialog_store.process_message` against the `Arc<RwLock<...>>` shared store. Result: every matching packet is parsed twice when `--api` is on in batch mode.
...
| `stats` | Mirrors `GET /v1/stats` from `output::api::get_stats` ([`src/output/api.rs:2018`](https://github.com/NormB/sipnab/blob/main/src/output/api.rs#L2018)) |

| Phase 8 infra | Reuses |
|---|---|
| Bind address parsing | `output::api::parse_bind_addr` ([`src/output/api.rs:499`](https://github.com/NormB/sipnab/blob/main/src/output/api.rs#L499)) |
...
| Rate limiting | `output::api::RateLimiter` ([`src/output/api.rs:263`](https://github.com/NormB/sipnab/blob/main/src/output/api.rs#L263)) |
...
| WebSocket / SSE Router mounting | Extend `output::api::build_router` ([`src/output/api.rs:448`](https://github.com/NormB/sipnab/blob/main/src/output/api.rs#L448)) with new routes; reuse the existing `guard()` middleware |
