...
[MCP server](mcp.md) introduces it.
...
| Capture from proxies I cannot install sipnab on | [Collect captures from several SIP servers in one place](mcp-estate.md#collect-captures-from-several-sip-servers-in-one-place) |
| Let agents outside my network reach it | [Reach sipnab from outside your network](mcp-estate.md#reach-sipnab-from-outside-your-network) |
| Point one agent at many capture hosts | [Query many capture hosts from one agent](mcp-estate.md#query-many-capture-hosts-from-one-agent) |
| **See one call cross an SBC, a proxy and a PBX** | [Follow one call across an SBC and its PBXes](mcp-estate.md#follow-one-call-across-an-sbc-and-its-pbxes) |
| Work out whether my SBC was a B2BUA on this call | [Read what matched, because the topology is not fixed](mcp-estate.md#read-what-matched-because-the-topology-is-not-fixed) |
| Decide whether to add a correlation identifier | [Choose a correlation identifier](mcp-estate.md#choose-a-correlation-identifier) |
...
2. **[server]** Verify the features:
   ...
   ```bash
   sipnab --version
   # sipnab 0.5.189 (...) features: native,tui,audio,tls,hep,api,mcp,mcp-http,metrics,plugins,vcon,bpf
...
2. **[server]** Create the unprivileged user the service runs as:
   ...
   feed HEP instead
   ([Collect captures from several SIP servers in one place (3)](mcp-estate.md#collect-captures-from-several-sip-servers-in-one-place)):
...
[Ask the SBC first](mcp-estate.md#ask-the-sbc-first) gives:
...
[`clients/python/mcp_probe.py`](../clients/python/mcp_probe.py) runs on every
reply:
...
[Read what matched](mcp-estate.md#read-what-matched-because-the-topology-is-not-fixed)
...
- HTTP wirings hand it a URL plus a bearer token. They are
  ...
  and [Reach sipnab from outside your network (4)](mcp-estate.md#reach-sipnab-from-outside-your-network).
...
[The tool reference](mcp-tools.md) carries the whole shape for every tool.
...
[the tool reference](mcp-tools.md#rtp_stats) lists every one.
...
`unanalysed_*` counters — [`capture_status`](mcp-tools.md#capture_status)
...
[`server_capabilities`](mcp-tools.md#server_capabilities) walks through it.
...
```json
{"jsonrpc":"2.0","id":1,"result":{"protocolVersion":"2025-06-18","capabilities":{"tools":{}},"serverInfo":{"name":"sipnab","version": "0.5.189"},"instructions":"sipnab MCP server — queries captured SIP dialogs ..."}}
```
...
This is [`clients/python/sipnab_mcp.py`](../clients/python/sipnab_mcp.py). CI
runs it against a committed capture with the SDK versions
[`clients/python/requirements-mcp.txt`](../clients/python/requirements-mcp.txt)
pins by hash.
...
This is [`clients/typescript/sipnab-mcp.ts`](../clients/typescript/sipnab-mcp.ts).
