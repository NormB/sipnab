...
2. **[server]** Verify the features:
   ...
   ```bash
   sipnab --version
   # sipnab 0.5.167 (...) features: native,tui,audio,tls,hep,api,mcp,mcp-http,metrics,plugins,vcon,bpf
   ```
...
```json
{"jsonrpc":"2.0","id":1,"result":{"protocolVersion":"2025-06-18","capabilities":{"tools":{}},"serverInfo":{"name":"sipnab","version": "0.5.167"},"instructions":"sipnab MCP server — queries captured SIP dialogs ..."}}
```
