+++
title = "Troubleshoot a call"
weight = 3
description = "Start from what the caller reports, find the matching section, and run the commands it gives, in order."
+++
<!-- Generated by scripts/build-site-pages.py from docs/troubleshooting.md — do not edit. Edit the source page and re-run the script; site_pages_mirror_is_current fails if this file is stale. -->
...
> |---|---|---|
> | `unsupported link type N` | The pcap's DLT has no decoder here. [Link types](@/docs/encapsulations.md#link-types) lists the ones sipnab reads, and `147` to `162` are the private-use `DLT_USER` range. | Convert it: `editcap -T ether in.pcap out.pcap`. If the link type is one sipnab should read, open an issue naming the number. |
...
>
> [Tuning capture on a busy server](@/docs/tuning-capture.md) covers how to read the two drop counters, what each one means, and what to change.
>
...
- A Markdown or JSON report of one call, for a ticket:
  [recipe 12](@/docs/cookbook.md#12-generate-a-call-report-text-markdown-json).
  A failure count per response code:
  [recipe 3](@/docs/cookbook.md#3-find-every-failed-call-grouped-by-response-code).
- The call's audio as a WAV, to hear what the caller heard:
  [recipe 13](@/docs/cookbook.md#13-export-rtp-audio-as-wav). It lists the codecs
...
- A capture read in the browser with nothing installed and nothing uploaded:
  [recipe 14](@/docs/cookbook.md#14-analyze-a-pcap-without-installing-anything).
...
- **SIP inside a UDP tunnel** (GTP-U, VXLAN, GENEVE). Off by default, and
  ...
  capture](@/docs/tuning-capture.md)).
...
The [CLI reference](@/docs/cli.md#archives-read-like-directories) covers the
...
from the TUI's capture-health panel ([recipe 6e](@/docs/cookbook.md#6e-find-who-feeds-the-collector-and-who-it-refuses)).
...
[`[crash]` config section](@/docs/config.md#crash) moves them. If you
