...
re-measured on a released artifact). The current release 0.5.40 (removal of the
legacy `s1` bearer-token format — an auth-path change, not a packet-path one)
changes neither path. The comparison
