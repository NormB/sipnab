...
re-measured on a released artifact). The current release 0.5.24 (TUI displayed-list rebuild floor —
an event-loop cache policy, not a packet-path change) changes neither path. The comparison
