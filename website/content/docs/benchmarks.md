...
re-measured on a released artifact). The current release 0.5.25 (churn-rebuild floors across all TUI
views — event-loop cache policy, not packet-path changes) changes neither path. The comparison
