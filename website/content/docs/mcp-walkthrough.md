...
2. **[server]** Verify the features:
   ...
   ```bash
   sipnab --version
   # sipnab 0.5.40 (...) features: native,tui,audio,tls,hep,api,mcp,mcp-http,metrics
   ```
...
- **Token hygiene.** Use `--mcp-token-file` (0600, root-owned) rather than
  ...
  Rotation/expiry via signed tokens is available ([authentication](@/docs/api.md#authentication)).
...
[the MCP reference's raw HTTP test](@/docs/mcp.md#raw-http-test).
