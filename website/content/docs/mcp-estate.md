+++
title = "Use MCP across many servers"
weight = 16
...
1. **[server]** Do [Step 0](@/docs/mcp-deploy.md#step-0-install-sipnab-every-server-once)
   ...
   [Keep a capture running between agent sessions (2B)](@/docs/mcp-deploy.md#keep-a-capture-running-between-agent-sessions),
   *without* the `setcap`.
...
2. **[server]** Install `/etc/systemd/system/sipnab-mcp.service` — this
   ...
   with [the SSH tunnel (2C)](@/docs/mcp-deploy.md#keep-a-capture-running-without-exposing-a-port).
   For [the token shape (2B)](@/docs/mcp-deploy.md#keep-a-capture-running-between-agent-sessions)
...
6. **[laptop]** Wire up exactly as steps 3–4 of
   [Keep a capture running without exposing a port (2C)](@/docs/mcp-deploy.md#keep-a-capture-running-without-exposing-a-port)
   (or steps 8–9 of
   [Keep a capture running between agent sessions (2B)](@/docs/mcp-deploy.md#keep-a-capture-running-between-agent-sessions)
...
- **Budget**: cap sipnab's footprint with `[limits]` (see
  [Understand the load on a busy server](@/docs/mcp-deploy.md#understand-the-load-on-a-busy-server))
...
Run [the token service (2B)](@/docs/mcp-deploy.md#keep-a-capture-running-between-agent-sessions),
[the SSH tunnel (2C)](@/docs/mcp-deploy.md#keep-a-capture-running-without-exposing-a-port),
...
1. **[each server]** Any persistent wiring above
   ([the token service (2B)](@/docs/mcp-deploy.md#keep-a-capture-running-between-agent-sessions)
   shown here).
...
[the SSH tunnel shape (2C)](@/docs/mcp-deploy.md#keep-a-capture-running-without-exposing-a-port)
...
For [the SSH stdio shape (2A)](@/docs/mcp-deploy.md#connect-claude-code-on-your-laptop-to-sipnab-on-a-server), `ssh -J bastion.example.net sbc-edge-1.example.net
...
> [the SSH stdio shape (2A)](@/docs/mcp-deploy.md#connect-claude-code-on-your-laptop-to-sipnab-on-a-server),
> [the token service (2B)](@/docs/mcp-deploy.md#keep-a-capture-running-between-agent-sessions)
> and [the SSH tunnel (2C)](@/docs/mcp-deploy.md#keep-a-capture-running-without-exposing-a-port)
