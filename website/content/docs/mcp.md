...
| [`search_by_time`](#search-by-time) | `start`, `end?`, `filter?`, `limit?` | Dialogs whose first message falls in an [RFC 3339](https://www.rfc-editor.org/rfc/rfc3339) window |
...
to [`get_message`](#get-message) for the parsed form.
...
> re-ask with a larger `limit`, and reach for [`list_dialogs`](#list-dialogs) or
> [`find_problems`](#find-problems) when the question is "how many", because
...
newest last. As with [`search_messages`](#search-messages), there is no total,
...
> [`server_capabilities`](#server-capabilities) and check the server's flags
...
[`list_dialogs`](#list-dialogs) with a DSL expression when you want a cursor
instead.
...
[`get_message`](#get-message) when the text reaches a model's context, and treat
