...
text with `-N`, NDJSON with `--json`, and the [MCP server](@/docs/mcp.md) for AI
agents. The [REST API](@/docs/api.md) serves the same data over HTTP. This page
...
- MCP tool responses (`get_dialog_report` with `format: "json"` — see the
  [MCP tool reference](@/docs/mcp-tools.md#get-dialog-report)), and
...
[`tests/schemas/notes.schema.json`](https://github.com/NormB/sipnab/blob/main/tests/schemas/notes.schema.json).
