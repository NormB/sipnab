...
[mcp.md](@/docs/mcp.md)) authenticate clients with
`Authorization: Bearer <token>`.
[cli-reference.md](@/docs/cli.md#network-listeners) lists every related flag.
...
|---|---|
| `503` | The client is over its per-IP rate budget (100 requests/second) or the in-flight cap (`--api-max-conn`). **The rate limiter runs before authentication**, so this answer arrives whether the credential is good, bad, or missing — see [rest-api.md](@/docs/api.md#bind-address-connection-limits). |
