+++
title = "Look up an MCP tool"
weight = 17
description = "Every MCP tool sipnab exposes, what question each answers, and the fields it returns."

[extra]
lead_in_body = true
+++
<!-- Generated by scripts/build-site-pages.py from docs/mcp-tools.md — do not edit. Edit the source page and re-run the script; site_pages_mirror_is_current fails if this file is stale. -->

Every tool the MCP server exposes, what it answers, and what it returns.

This is lookup material, not reading material.

- [MCP server](@/docs/mcp.md) — what MCP is, and a first working example.
- [MCP deployment](@/docs/mcp-deploy.md) — the deployment shapes.
- [MCP protocol](@/docs/mcp-protocol.md) — the wire contract, security model and error semantics.
...
| [`runtime_stats`](#runtime-stats) | `sample_seconds?` | What sipnab is doing and what it costs the host: its own memory, threads, descriptors and CPU; the host's totals and the basis they came from; its share of them and whether that share is load-bearing; per-interface counters; store occupancy against the caps |
...
| [`hep_senders`](#hep-senders) | `limit?` | Who feeds this run's HEP listener: each sender, its count, when it was last heard and whether it went silent, and every address the listener refused, by reason |
...
| [`server_capabilities`](#server-capabilities) | -- | sipnab version, the optional features this binary carries, and the libpcap it runs |
...
| [`await_condition`](#await-condition) | `filter`, `timeout_seconds?`, `poll_interval_ms?` | Waits until a filter matches or a deadline passes, whichever is first, so a live capture costs one call instead of a polling loop |
...
| [`siprec_metadata`](#siprec-metadata) | `call_id` | The SIPREC ([RFC 7866](https://www.rfc-editor.org/rfc/rfc7866)) recording metadata a call carried: session id, mode, participants with AOR and display name, and the recorded streams, each naming its `m=` line label and the participant that sends it |
...
| [`decode_ng`](#decode-ng) | `frame_ref` | Decodes the relay control message one pointer names: the command, the call, whether it carries SDP, and which delivery path carried it |
...
| Tool | Parameters | Returns |
|---|---|---|
| [`export_capture`](#export-capture) | `filename` | Writes held SIP signaling to a pcap in `--mcp-file-root` (re-synthesized frames, no RTP) |
...
| [`export_vcon`](#export-vcon) | `call_id?`, `filter?`, `limit?` | Dialogs as vCon conversation containers, structured JSON, each with its SHA-256. One `call_id` or a whole filtered set. Unsigned, retained audio inline, and every omission stated in the response |
| [`validate_vcon`](#validate-vcon) | `call_id?`, `container?` | Checks a container against the schema sipnab vendors and names the one documented deviation instead of passing it |
...
| Tool | Parameters | Returns |
|---|---|---|
| [`query_relay`](#query-relay) | `call_id?`, `max_calls?` | **Transmits.** Asks the configured relay what it holds right now. Needs `--mcp-allow-relay-query` and a live source; the destination comes from operator configuration only |
| [`relay_stats`](#relay-stats) | `call_id?`, `names_only?` | **Transmits.** Asks the configured relay for its own counters (global, per-call, or the names it knows), tiered `relay_reported`. Same opt-in and live-source gate as `query_relay` |
| [`relay_compare`](#relay-compare) | `call_id` | **Transmits.** Sets the relay's per-call RTP count beside sipnab's measured count, both tiers named and a word verdict, never a sum. Same gate as `query_relay` |
...
call. [Untrusted capture text](@/docs/mcp-protocol.md#untrusted-capture-text) gives the per-tool
...
[`security_findings`](#security-findings). It costs two literal prefix tests on
...
and the ids they claim are [`hep_senders`](#hep-senders)' answer. Without a
...
The YANG section of [Output formats](@/docs/output-formats.md) lists every difference
...
turn. Opening one is not free: [`open_capture`](#open-capture) replaces the
...
[`show_evidence`](#show-evidence). `from_user` and `to_user` arrive fenced,
...
| `capture_identity` | object | Which capture answered — see [`capture_status`](#capture-status). |
| `schema_version` | u32 | `1` for this shape. |
...
[`aggregate_dialogs`](#aggregate-dialogs) answers "how many" and stops.
...
| `filter` | string? | Alias or DSL, applied before grouping — the same vocabulary [`list_dialogs`](#list-dialogs) takes. | No filter — the whole store. |
...
PERCENTS here and in [`evaluate_expectations`](#evaluate-expectations), which
...
One dimension, for the reason [`aggregate_dialogs`](#aggregate-dialogs) gives:
...
an artifact can reach](@/docs/install.md#which-capture-backends-an-artifact-can-reach)
...
| `fields` | string[]? | Field names to keep on each row, plus `call_id`. Same rules as [`list_dialogs`](#list-dialogs) — see [Narrowing a row with `fields`](#narrowing-a-row-with-fields). | The whole row. |
...
and `message_index` straight to [`get_message`](#get-message) for the parsed
form.
...
**a narrower row than [`list_dialogs`](#list-dialogs) returns**, with no
...
The DSL narrows [`list_dialogs`](#list-dialogs), [`find_problems`](#find-problems),
[`search_by_time`](#search-by-time), [`aggregate_dialogs`](#aggregate-dialogs)
and [`group_dialogs`](#group-dialogs), and before this tool every way of trying
...
| Name | Type | Legal values | If omitted |
|---|---|---|---|
| `expr` | string | A raw [filter DSL](@/docs/filter-dsl.md) expression, **or** one of the diagnostic aliases [`list_dialogs`](#list-dialogs) takes. Nothing you can put here fails the call. | Required — the call fails. |
...
and [`list_dialogs`](#list-dialogs) already pages the whole history oldest-first
...
[`security_findings`](#security-findings) uses. Only on a server with a detector
...
straight into [`diagnose_registration`](#diagnose-registration).
...
whichever happens first. One call replaces a [`tail_dialogs`](#tail-dialogs)
loop in which every turn that finds nothing still costs a model call.

| Name | Type | Legal values | If omitted |
|---|---|---|---|
| `filter` | string | A named alias or a filter DSL expression, exactly what [`list_dialogs`](#list-dialogs) accepts. [`validate_filter`](#validate-filter) checks one without waiting. | Required — the call fails. |
...
same summary rows [`list_dialogs`](#list-dialogs) returns, fencing included,
...
[`list_dialogs`](#list-dialogs) with the same `filter` is the paging surface,
...
MCP surface reached it. [`find_correlated`](#find-correlated) answers one hop
...
[`get_dialog`](#get-dialog) hands back a quarter of the call without saying so.
...
`strategy` vocabulary [`find_correlated`](#find-correlated) tabulates. The root
...
[`aggregate_dialogs`](#aggregate-dialogs) groups, and ranks the buckets by how
far they MOVED.
...
[`get_capture_report`](#get-capture-report) reports. It destroys nothing, and
something observable moves. This reuses
[`open_capture`](#open-capture)'s existing behavior rather than inventing a new
one.
...
[`aggregate_dialogs`](#aggregate-dialogs) nine.
[`group_dialogs`](#group-dialogs)'s three extras — `to_domain`, `hour`,
...
returned by [`list_dialogs`](#list-dialogs). An unknown one fails with
...
| Field | Type | Description |
|---|---|---|
| `dialog` | object | The same summary [`list_dialogs`](#list-dialogs) returns, with `from_user` and `to_user` fenced. |
...
> [`get_message`](#get-message) gives them. The SDP body keeps its line
...
[`show_evidence`](#show-evidence) when you need those.
...
[`render_ladder`](#render-ladder) produces for the same dialog. All three append
...
[`get_capture_report`](#get-capture-report) when the capture was not read in
...
[`triage_call`](#triage-call) documents above. It is absent from the example
...
This report bundles what [`triage_call`](#triage-call),
[`get_sdp_timeline`](#get-sdp-timeline) and [`rtp_stats`](#rtp-stats) answer
...
`msg_count` on any [`list_dialogs`](#list-dialogs) row is the count, so the last
valid index is `msg_count - 1`. [`get_dialog`](#get-dialog) pages messages from
its `cursor`, so the Nth row of that page sits at `cursor + N`, and every
[`lint_dialog`](#lint-dialog) finding reports the `message_index` it fired on.
...
Returns one message in the same shape [`get_dialog`](#get-dialog) uses for a
...
`dscp` and `input_origin` carry the meanings [`get_dialog`](#get-dialog) gives
...
[`decode_evidence`](#decode-evidence), which needs a server-side
...
already tracks — see [deferred-and-declined.md](https://github.com/NormB/sipnab/blob/main/docs/design/deferred-and-declined.md#the-prompt-injection-chain-grounded)
...
[`triage_call`](#triage-call) calls this capture one-way media rather than a
fault.
...
behind the verdict, ready for [`get_message`](#get-message). The example runs
against
...
[SIP conformance rules](@/docs/sip-lint-rules.md) lists every rule, the section behind
...
[`show_evidence`](#show-evidence) and the claim becomes bytes a reader can
...
rules](@/docs/sip-lint-rules.md) documents the pattern syntax.
...
| `suppression_file` | string? | A bare filename inside `--mcp-file-root`, exactly as [`lint_dialog`](#lint-dialog) takes it. Wins outright over the discovery walk. | sipnab walks for a `.sipnablint` beside the capture and upward. |
...
it to [`show_evidence`](#show-evidence) to read the bytes behind the claim.
...
as [`lint_dialog`](#lint-dialog) reports them.
...
| `suppression_file` | string? | A lint suppression file in `--mcp-file-root`, applied to the `lint_errors` rules. | Discovers a `.sipnablint` beside the capture, exactly as [`lint_dialog`](#lint-dialog) does. |
...
[`group_dialogs`](#group-dialogs) is a PERCENT. A threshold of `0.99` means 99%
...
> [`server_capabilities`](#server-capabilities) is not necessary for this
> question: the answer is in the response.

The page fields carry the meanings they do everywhere else on this surface —
[`search_messages`](#search-messages) documents the same four. `armed_kinds` is
...
| Name | Type | Legal values | If omitted |
|---|---|---|---|
| `finding_id` | string | `<rule_name>@<src_ip>@<timestamp>` — the three fields [`security_findings`](#security-findings) already returns for every finding. | Required — the call fails. |
...
[`security_findings`](#security-findings). The scan walks the whole ring buffer
...
[`list_dialogs`](#list-dialogs) returned for the first call in
[`tests/pcap-samples/sip-rtp-g711.pcap`](https://github.com/NormB/sipnab/raw/main/tests/pcap-samples/sip-rtp-g711.pcap),
...
[`show_evidence`](#show-evidence) answers half the provenance question — are
...
[`list_dialogs`](#list-dialogs) returns for the first call in
[`tests/pcap-samples/sip-rtp-g711.pcap`](https://github.com/NormB/sipnab/raw/main/tests/pcap-samples/sip-rtp-g711.pcap):
...
mechanism exists to prevent. [`get_message`](#get-message) carries the message's
...
[`export_capture`](#export-capture)'s writer and carry that tool's whole
...
The ladder and the stats come from [`render_ladder`](#render-ladder) and
[`rtp_stats`](#rtp-stats) rather than from a second rendering path, so a package
...
[`explain_attribution`](#explain-attribution). `on_believed_mirror_port` stands
...
[`decode_evidence`](#decode-evidence), and a pointer leading nowhere returns
...
[`compare_captures`](#compare-captures) and
[`build_evidence_package`](#build-evidence-package) all require
`--mcp-file-root <DIR>` and refuse to run without it. They take a **bare
filename, never a path**. [`generate_repro`](#generate-repro) joins them the
moment you give it a `filename`, and
[`decode_evidence`](#decode-evidence) pushes the source half of a frame pointer
...
> there for [`open_capture`](#open-capture), and every earlier export, which
...
[`list_captures`](#list-captures) then never lists. No optional parameters.
...
[`server_capabilities`](#server-capabilities) lists what a given binary carries.
...
**Transmits, like [`query_relay`](#query-relay).** It asks the configured relay
...
measured. That comparison is [`relay_compare`](#relay-compare).
...
**Transmits, like [`query_relay`](#query-relay).** It sets the relay's own RTP
...
described in [Archives](@/docs/cli.md#archives). When members stay locked,
