...
[`estimate_mos()`](https://github.com/NormB/sipnab/blob/main/src/rtp/quality.rs) is an E-model computation from
...
| `cumulative_lost` has no sign bit | A 24-bit **signed** field zero-extended: net-duplicate streams reported ~16.7M lost packets | Sign extension in [`rtcp.rs`](https://github.com/NormB/sipnab/blob/main/src/rtp/rtcp.rs); the sign is preserved, so a net-duplicate stream reads as a small negative rather than "no loss" |
