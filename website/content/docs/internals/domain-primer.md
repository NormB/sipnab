...
title = "Domain Primer"
weight = 1
description = "The SIP and RTP model the code assumes you already have — written for the Rust engineer who is new to VoIP."

[extra]
has_diagrams = true
+++
<!-- Generated by scripts/build-site-internals.py from docs/internals/domain-primer.md — do not edit. Edit the source page and re-run the script; dev_docs_drift_test fails if this file is stale. -->
...
because [`SipDialog::method`](https://github.com/NormB/sipnab/blob/main/src/sip/dialog.rs) takes its value once at creation
...
This is not pedantry. [`update_timing()`](https://github.com/NormB/sipnab/blob/main/src/sip/timing.rs) records the
...
[`SipDialog`](https://github.com/NormB/sipnab/blob/main/src/sip/dialog.rs) therefore treats 401/407 as
...
`pdd_ms()` in [`timing.rs`](https://github.com/NormB/sipnab/blob/main/src/sip/timing.rs) measures INVITE → first
...
That is why [`StreamStore`](https://github.com/NormB/sipnab/blob/main/src/rtp/stream_store.rs) is a first-class
...
([`stream.rs`](https://github.com/NormB/sipnab/blob/main/src/rtp/stream.rs)). A plain `>` comparison reports one
...
[`estimate_mos_with_delay()`](https://github.com/NormB/sipnab/blob/main/src/rtp/quality.rs) is an E-model
...
[`MosDelay`](https://github.com/NormB/sipnab/blob/main/src/rtp/quality.rs) resolves it per stream and every surface
...
[`analyze_burst_gap()`](https://github.com/NormB/sipnab/blob/main/src/rtp/quality.rs) classifies which it is, and
...
[`diagnose_media()`](https://github.com/NormB/sipnab/blob/main/src/rtp/diagnosis.rs) is where sipnab untangles this:
...
|---|---|---|
| A 200 OK answers the INVITE | A re-INVITE's 200 overwrote `answered_at`; held calls reported absurd setup times | `invite_cseq` pinning in [`timing.rs`](https://github.com/NormB/sipnab/blob/main/src/sip/timing.rs) |
| RTCP jitter is milliseconds | Reported jitter off by the clock-rate factor, and MOS fed the wrong units | Clock-rate conversion in [`stream_store.rs`](https://github.com/NormB/sipnab/blob/main/src/rtp/stream_store.rs) |
| `cumulative_lost` has no sign bit | A 24-bit **signed** field zero-extended: net-duplicate streams reported ~16.7M lost packets | Sign extension in [`rtcp.rs`](https://github.com/NormB/sipnab/blob/main/src/rtp/rtcp.rs), keeping the sign, so a net-duplicate stream reads as a small negative rather than "no loss" |
| Transit deltas never go negative | One reordered packet underflowed to a ~4.29e9 jitter spike | Signed `i32` delta in [`stream.rs`](https://github.com/NormB/sipnab/blob/main/src/rtp/stream.rs) |
| Sequence numbers only increase | A "loss burst" of 65,000 packets once per wrap | `wrapping_add` comparison in [`stream.rs`](https://github.com/NormB/sipnab/blob/main/src/rtp/stream.rs) |
| SDP role follows message type | Delayed-offer calls labeled backwards | [`determine_offer_answer()`](https://github.com/NormB/sipnab/blob/main/src/sip/sdp_timeline.rs) |
| 401/407 is a failure | Most of a healthy carrier's calls reported as failed | Non-challenge maximum in [`dialog.rs`](https://github.com/NormB/sipnab/blob/main/src/sip/dialog.rs) |
