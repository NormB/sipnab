...
| `wiki-sync.yml` | push to main (path-filtered) | Regenerates the wiki from `docs/` via [`scripts/build-wiki.py`](https://github.com/NormB/sipnab/blob/main/scripts/build-wiki.py). |
...
[`ops/tsan/suppressions.txt`](https://github.com/NormB/sipnab/blob/main/ops/tsan/suppressions.txt) silences libpcap, libasound and the dynamic loader,
...
`--cfg sipnab_tsan`, and [`src/main.rs`](https://github.com/NormB/sipnab/blob/main/src/main.rs) skips its `#[global_allocator]` under that
...
All of that lives in [`ops/tsan/verdict.sh`](https://github.com/NormB/sipnab/blob/main/ops/tsan/verdict.sh), not inline in the workflow, with
[`ops/tsan/test-verdict.sh`](https://github.com/NormB/sipnab/blob/main/ops/tsan/test-verdict.sh) beside it and a `tsan-verdict` job in `ci.yml`
...
`quality.yml`'s `Docs` job runs three tools over `docs/`, [`website/content/`](https://github.com/NormB/sipnab/blob/main/website/content) and
...
graph, and the `third_party_notices_are_current` gate in [`tests/docs_drift_test.rs`](https://github.com/NormB/sipnab/blob/main/tests/docs_drift_test.rs)
...
and both path lists live in [`scripts/prose-gates.sh`](https://github.com/NormB/sipnab/blob/main/scripts/prose-gates.sh), sourced by
`pre-commit`, `pre-push` and [`scripts/preflight.sh`](https://github.com/NormB/sipnab/blob/main/scripts/preflight.sh). Each caller keeps only
...
push-and-wait feedback loop. [`scripts/check-feature-matrix.py`](https://github.com/NormB/sipnab/blob/main/scripts/check-feature-matrix.py) now runs all
fifteen locally, reading both the combo list and `RUSTFLAGS` out of
[`.github/workflows/ci.yml`](https://github.com/NormB/sipnab/blob/main/.github/workflows/ci.yml) so the two cannot drift.

[`pre-push`](https://github.com/NormB/sipnab/blob/main/.githooks/pre-push) adds thirteen hard gates that `cargo test`
...
The YANG gate, [`scripts/check-yang.py`](https://github.com/NormB/sipnab/blob/main/scripts/check-yang.py), exists
because a test generates the module under `yang/`: [`tests/yang_module_test.rs`](https://github.com/NormB/sipnab/blob/main/tests/yang_module_test.rs)
...
[`scripts/requirements-yang.txt`](https://github.com/NormB/sipnab/blob/main/scripts/requirements-yang.txt)) and runs the script with
...
[`scripts/check-non-linux.sh`](https://github.com/NormB/sipnab/blob/main/scripts/check-non-linux.sh) copies
...
`capture::uprobe` in [`src/capture/native.rs`](https://github.com/NormB/sipnab/blob/main/src/capture/native.rs) did precisely that, and were green
...
[`install-from-source.sh`](https://github.com/NormB/sipnab/blob/main/scripts/install-from-source.sh) has nothing to do with
...
[`website/config.toml`](https://github.com/NormB/sipnab/blob/main/website/config.toml) carries both: `version`, which the Pages step overwrites
...
[`scripts/publish-crates.py`](https://github.com/NormB/sipnab/blob/main/scripts/publish-crates.py).
...
are different jobs. [`scripts/check-cert-expiry.sh`](https://github.com/NormB/sipnab/blob/main/scripts/check-cert-expiry.sh) answers the other half, and
...
`published_macos_floors_match_the_toolchain` holds [`website/config.toml`](https://github.com/NormB/sipnab/blob/main/website/config.toml) to what
...
[`scripts/build-third-party-notices.py`](https://github.com/NormB/sipnab/blob/main/scripts/build-third-party-notices.py). Let that constant say
...
Version strings live in `Cargo.toml`, [`website/config.toml`](https://github.com/NormB/sipnab/blob/main/website/config.toml), [`man/sipnab.1`](https://github.com/NormB/sipnab/blob/main/man/sipnab.1),
[`fuzz/Cargo.lock`](https://github.com/NormB/sipnab/blob/main/fuzz/Cargo.lock) and several docs, and committing with any of them out of step
...
The same mechanism gates the test count in [`website/templates/index.html`](https://github.com/NormB/sipnab/blob/main/website/templates/index.html), by
...
[`website/config.toml`](https://github.com/NormB/sipnab/blob/main/website/config.toml), asserted by `site_release_date_matches_changelog`.
...
[the benchmarks page](@/docs/benchmarks.md) records one occasion that fooled
