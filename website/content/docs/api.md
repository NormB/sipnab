...
The same dialog / RTP / diagnostic data is also exposed to AI agents over the Model Context Protocol; see [MCP Server](@/docs/mcp.md). The MCP path uses the same in-memory stores as this REST API, so one running sipnab instance can serve both surfaces simultaneously.

All API flags are catalogued in [CLI Reference](@/docs/cli.md#network-listeners).
