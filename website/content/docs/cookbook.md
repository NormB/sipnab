...
has_diagrams = true
+++
<!-- Generated by scripts/build-site-pages.py from docs/examples.md — do not edit. Edit the source page and re-run the script; site_pages_mirror_is_current fails if this file is stale. -->
...
| Understand what reading TLS without keys costs and requires | [TLS-without-keys walkthrough](@/docs/uprobe-walkthrough.md) — security implications, and whether your kernel supports it |
...
- A `--snaplen` that cut the frame short is a different problem with the same symptom. Reassembly cannot restore bytes the capture never took ([22. Measure whether the loss is yours or the network's](#22-measure-whether-the-loss-is-yours-or-the-network-s)).
...
- A browser leg carries no RTP that a server-side tap can see when the media goes peer to peer. Put the capture where the media is, or read the relay's account of it ([54. Find out where a stream's endpoint came from](#54-find-out-where-a-stream-s-endpoint-came-from)).
...
More in [output-formats.md](@/docs/output-formats.md).
...
- [filter-dsl.md](@/docs/filter-dsl.md) — the full filter language behind `--filter`
- [mcp.md](@/docs/mcp.md) — drive the same analysis from an AI agent
