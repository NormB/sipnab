...
Manual download with checksum verification (replace `<version>` with the latest, e.g. 0.5.4):
...
```bash
sudo rpm -i sipnab-<version>-1.x86_64.rpm  # replace <version> with the latest, e.g. 0.5.4
...
sipnab 0.5.4 (c9620a5f) features: native,tui,audio,tls,hep,api,mcp,mcp-http
