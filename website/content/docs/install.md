+++
title = "Install sipnab"
weight = 1
description = "Install sipnab from a pre-built binary, a package, Homebrew or cargo, then check the download is genuine."
+++
<!-- Generated by scripts/build-site-pages.py from docs/install.md — do not edit. Edit the source page and re-run the script; site_pages_mirror_is_current fails if this file is stale. -->
...
| `sipnab-<version>-<target>.debug` | per target | — | Linux symbol file for the binary of the same name, for reading a [crash report](@/docs/troubleshooting.md#send-us-a-crash-report) or a core dump. One per tarball, plus `-noaudio` ones for the `-noaudio` packages |
...
MCP [`server_capabilities`](@/docs/mcp-tools.md#server-capabilities) tool and of the
REST [`GET /v1/capabilities`](@/docs/api.md#get-v1-capabilities) route, and it
...
Claude Desktop, …), start with [MCP server](@/docs/mcp.md), which covers the `mcp` and
`mcp-http` features and a first working example.
[Connect an AI agent to sipnab](@/docs/mcp-deploy.md) covers running it on a server,
...
[crash report](@/docs/troubleshooting.md#send-us-a-crash-report) or a core dump.
...
- [examples.md](@/docs/cookbook.md) — copy-paste recipes for the most common tasks
- [keybindings.md](@/docs/keybindings.md) — driving the interactive TUI
- [cli-reference.md](@/docs/cli.md) — every flag, for headless use
