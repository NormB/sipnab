...
Manual download with checksum verification (replace `<version>` with the latest, e.g. 0.5.30):
...
```bash
sudo rpm -i sipnab-<version>-1.x86_64.rpm  # replace <version> with the latest, e.g. 0.5.30
