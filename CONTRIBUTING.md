...
| `python3 scripts/check-yang.py` | The `sipnab-diagnosis` YANG module is generated from the analysis's tables, and a Rust test proves the committed file matches them; only a YANG implementation can say it is valid YANG. `pyang --lint` and `yanglint` compile it, and `pyang --check-update-from` holds a new revision to the last. `NOT CHECKED` where neither tool is installed; CI installs both and fails without them. |
